...
class UserPermissionArgs {
  ...
  UserPermissionArgs({
    List<UserPermissionPermission>? permissions,
    required String subAccountUserId,
  }) :
  ...
  factory UserPermissionArgs.fromMap(Map<String, dynamic> map) {
    return UserPermissionArgs(
      permissions: map['permissions'] == null ? null : pulumi.Input.decodeList<UserPermissionPermission>(map['permissions'], (value) => UserPermissionPermission.fromMap((value as Map).cast<String, dynamic>())),
      subAccountUserId: map['subAccountUserId'] as String,
    );
  }
}
