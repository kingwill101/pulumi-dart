...
class Union<T0, T1> {
  ...
  /// Returns value as `T0`, or throws if union holds `T1`.
  T0 get asT0 => _which == 0 ? _t0! : throw StateError('Not of type T0');

  /// Returns value as `T1`, or throws if union holds `T0`.
  T1 get asT1 => _which == 1 ? _t1! : throw StateError('Not of type T1');

  /// Whether union currently holds `T0`.
  bool get isT0 => _which == 0;

  /// Whether union currently holds `T1`.
  bool get isT1 => _which == 1;

  /// Raw active value (`T0` or `T1`).
  dynamic get value => _which == 0 ? _t0 : _t1;

  /// Pattern-matches the active branch and returns the mapped result.
