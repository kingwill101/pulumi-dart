...
class EC2ServiceArgs {
  ...
  final pulumi.Input<bool?>? useClusterDefaultCapacityProviderStrategy;
  ...
  /// [capacityProviderStrategies] Capacity provider strategies to use for the service. Can be one or more. Updating this argument requires `forceNewDeployment = true`. See below. Conflicts with `launchType`.
  ...
  /// [forceNewDeployment] Enable to force a new task deployment of the service. This can be used to update tasks to use a newer Docker image with same image/tag combination (e.g., `myimage:latest`), roll Fargate tasks onto a newer platform version, or immediately deploy `orderedPlacementStrategy` and `placementConstraints` updates.
  ...
  /// [placementConstraints] Rules that are taken into consideration during task placement. Updates to this configuration will take effect next task deployment unless `forceNewDeployment` is enabled. Maximum number of `placementConstraints` is `10`. See below.
  ...
  /// [serviceRegistries] Service discovery registries for the service. The maximum number of `serviceRegistries` blocks is `1`. See below.
  ...
  /// [useClusterDefaultCapacityProviderStrategy] If `true`, this service will use the cluster's default capacity provider strategy. When enabled, this provider omits both `launchType` and `capacityProviderStrategies` from the ECS service. Only one of [useClusterDefaultCapacityProviderStrategy] or [capacityProviderStrategies] can be provided. The cluster must have a default capacity provider strategy configured, or ECS service creation or update will fail.
  ...
  /// [vpcLatticeConfigurations] VPC Lattice configuration for your service that allows Lattice to connect, secure, and monitor your service across multiple accounts and VPCs. See below.
  const EC2ServiceArgs({
    this.alarms,
    ...
    this.triggers,
    this.useClusterDefaultCapacityProviderStrategy,
  ...
  factory EC2ServiceArgs.fromMap(Map<String, dynamic> map) {
    return EC2ServiceArgs(
      ...
      deploymentMaximumPercent: (() { final guardedValue = map['deploymentMaximumPercent']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      ...
      healthCheckGracePeriodSeconds: (() { final guardedValue = map['healthCheckGracePeriodSeconds']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      ...
      useClusterDefaultCapacityProviderStrategy: (() { final guardedValue = map['useClusterDefaultCapacityProviderStrategy']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as bool); })(),
