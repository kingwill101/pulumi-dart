...
class DatabaseKafkaTopicConfig {
  ...
  /// [maxCompactionLagMs] The maximum time, in ms, that a particular message will remain uncompacted. This will not apply if the `compressionType` is set to "uncompressed" or it is set to `producer` and the producer is not using compression.
  ...
  /// [messageTimestampType] Specifies which timestamp to use for the message. This may be one of "createTime" or "logAppendTime".
  /// [minCleanableDirtyRatio] A scale between 0.0 and 1.0 which controls the frequency of the compactor. Larger values mean more frequent compactions. This is often paired with `maxCompactionLagMs` to control the compactor frequency.
