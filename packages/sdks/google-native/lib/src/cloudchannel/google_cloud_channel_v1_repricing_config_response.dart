...
class GoogleCloudChannelV1RepricingConfigResponse {
  ...
  factory GoogleCloudChannelV1RepricingConfigResponse.fromMap(Map<String, dynamic> map) {
    return GoogleCloudChannelV1RepricingConfigResponse(
      adjustment: (GoogleCloudChannelV1RepricingAdjustmentResponse.fromMap((map['adjustment'] as Map).cast<String, dynamic>())).input(),
      channelPartnerGranularity: ((map['channelPartnerGranularity'] as Map).cast<String, dynamic>()).input(),
      conditionalOverrides: (pulumi.Input.decodeList<GoogleCloudChannelV1ConditionalOverrideResponse>(map['conditionalOverrides'], (value) => GoogleCloudChannelV1ConditionalOverrideResponse.fromMap((value as Map).cast<String, dynamic>()))).input(),
      effectiveInvoiceMonth: (GoogleTypeDateResponse.fromMap((map['effectiveInvoiceMonth'] as Map).cast<String, dynamic>())).input(),
      entitlementGranularity: (GoogleCloudChannelV1RepricingConfigEntitlementGranularityResponse.fromMap((map['entitlementGranularity'] as Map).cast<String, dynamic>())).input(),
      rebillingBasis: (map['rebillingBasis'] as String).input(),
    );
  }
}
