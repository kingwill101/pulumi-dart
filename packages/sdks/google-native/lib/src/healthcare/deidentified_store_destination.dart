...
class DeidentifiedStoreDestination {
  ...
  const DeidentifiedStoreDestination({
    this.config,
    this.store,
  });
  ...
  factory DeidentifiedStoreDestination.fromMap(Map<String, dynamic> map) {
    return DeidentifiedStoreDestination(
      config: (() { final guardedValue = map['config']; if (guardedValue == null) return null; return pulumi.Input.fromValue(DeidentifyConfig.fromMap((guardedValue as Map).cast<String, dynamic>())); })(),
