...
class HivePartitioningOptionsResponse {
  ...
  /// Creates a new [HivePartitioningOptionsResponse].
  /// [fields] For permanent external tables, this field is populated with the hive partition keys in the order they were inferred. The types of the partition keys can be deduced by checking the table schema (which will include the partition keys). Not every API will populate this field in the output. For example, Tables.Get will populate it, but Tables.List will not contain this field.
  ...
  factory HivePartitioningOptionsResponse.fromMap(Map<String, dynamic> map) {
    return HivePartitioningOptionsResponse(
      fields: ((map['fields'] as List).cast<String>()).input(),
      mode: (map['mode'] as String).input(),
      requirePartitionFilter: (map['requirePartitionFilter'] as bool).input(),
      sourceUriPrefix: (map['sourceUriPrefix'] as String).input(),
    );
  }
}
