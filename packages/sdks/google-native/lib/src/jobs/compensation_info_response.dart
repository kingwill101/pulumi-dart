...
class CompensationInfoResponse {
  ...
  factory CompensationInfoResponse.fromMap(Map<String, dynamic> map) {
    return CompensationInfoResponse(
      annualizedBaseCompensationRange: (CompensationRangeResponse.fromMap((map['annualizedBaseCompensationRange'] as Map).cast<String, dynamic>())).input(),
      annualizedTotalCompensationRange: (CompensationRangeResponse.fromMap((map['annualizedTotalCompensationRange'] as Map).cast<String, dynamic>())).input(),
      entries: (pulumi.Input.decodeList<CompensationEntryResponse>(map['entries'], (value) => CompensationEntryResponse.fromMap((value as Map).cast<String, dynamic>()))).input(),
    );
  }
}
