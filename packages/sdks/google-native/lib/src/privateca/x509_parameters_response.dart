...
class X509ParametersResponse {
  ...
  factory X509ParametersResponse.fromMap(Map<String, dynamic> map) {
    return X509ParametersResponse(
      additionalExtensions: (pulumi.Input.decodeList<X509ExtensionResponse>(map['additionalExtensions'], (value) => X509ExtensionResponse.fromMap((value as Map).cast<String, dynamic>()))).input(),
      aiaOcspServers: ((map['aiaOcspServers'] as List).cast<String>()).input(),
      caOptions: (CaOptionsResponse.fromMap((map['caOptions'] as Map).cast<String, dynamic>())).input(),
      keyUsage: (KeyUsageResponse.fromMap((map['keyUsage'] as Map).cast<String, dynamic>())).input(),
      nameConstraints: (NameConstraintsResponse.fromMap((map['nameConstraints'] as Map).cast<String, dynamic>())).input(),
      policyIds: (pulumi.Input.decodeList<ObjectIdResponse>(map['policyIds'], (value) => ObjectIdResponse.fromMap((value as Map).cast<String, dynamic>()))).input(),
    );
  }
}
