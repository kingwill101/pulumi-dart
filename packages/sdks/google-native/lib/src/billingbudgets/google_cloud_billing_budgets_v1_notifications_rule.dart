...
class GoogleCloudBillingBudgetsV1NotificationsRule {
  ...
  /// Creates a new [GoogleCloudBillingBudgetsV1NotificationsRule].
  ...
  /// [pubsubTopic] Optional. The name of the Pub/Sub topic where budget-related messages are published, in the form `projects/{project_id}/topics/{topic_id}`. Updates are sent to the topic at regular intervals; the timing of the updates is not dependent on the [threshold rules](#thresholdrule) you've set. Note that if you want your [Pub/Sub JSON object](https://cloud.google.com/billing/docs/how-to/budgets-programmatic-notifications#notification_format) to contain data for `alertThresholdExceeded`, you need at least one [alert threshold rule](#thresholdrule). When you set threshold rules, you must also enable at least one of the email notification options, either using the default IAM recipients or Cloud Monitoring email notification channels. To use Pub/Sub topics with budgets, you must do the following: 1. Create the Pub/Sub topic before connecting it to your budget. For guidance, see [Manage programmatic budget alert notifications](https://cloud.google.com/billing/docs/how-to/budgets-programmatic-notifications). 2. Grant the API caller the `pubsub.topics.setIamPolicy` permission on the Pub/Sub topic. If not set, the API call fails with PERMISSION_DENIED. For additional details on Pub/Sub roles and permissions, see [Permissions required for this task](https://cloud.google.com/billing/docs/how-to/budgets-programmatic-notifications#permissions_required_for_this_task).
  /// [schemaVersion] Optional. Required when NotificationsRule.pubsub_topic is set. The schema version of the notification sent to NotificationsRule.pubsub_topic. Only "1.0" is accepted. It represents the JSON schema as defined in https://cloud.google.com/billing/docs/how-to/budgets-programmatic-notifications#notification_format.
  ...
  factory GoogleCloudBillingBudgetsV1NotificationsRule.fromMap(Map<String, dynamic> map) {
    return GoogleCloudBillingBudgetsV1NotificationsRule(
      disableDefaultIamRecipients: map['disableDefaultIamRecipients'] == null ? null : map['disableDefaultIamRecipients'] as bool,
      enableProjectLevelRecipients: map['enableProjectLevelRecipients'] == null ? null : map['enableProjectLevelRecipients'] as bool,
      monitoringNotificationChannels: map['monitoringNotificationChannels'] == null ? null : (map['monitoringNotificationChannels'] as List).cast<String>(),
