...
class DynamicGroupMetadataResponse {
  ...
  factory DynamicGroupMetadataResponse.fromMap(Map<String, dynamic> map) {
    return DynamicGroupMetadataResponse(
      queries: pulumi.Input.fromValue(pulumi.Input.decodeList<DynamicGroupQueryResponse>(map['queries']!, (value) => DynamicGroupQueryResponse.fromMap((value as Map).cast<String, dynamic>()))),
      status: pulumi.Input.fromValue(DynamicGroupStatusResponse.fromMap((map['status']! as Map).cast<String, dynamic>())),
    );
  }
}
