...
class GoogleCloudMlV1HyperparameterSpecResponse {
  ...
  factory GoogleCloudMlV1HyperparameterSpecResponse.fromMap(Map<String, dynamic> map) {
    return GoogleCloudMlV1HyperparameterSpecResponse(
      ...
      enableTrialEarlyStopping: (map['enableTrialEarlyStopping'] as bool).input(),
      goal: (map['goal'] as String).input(),
      hyperparameterMetricTag: (map['hyperparameterMetricTag'] as String).input(),
      maxFailedTrials: (map['maxFailedTrials'] as int).input(),
      maxParallelTrials: (map['maxParallelTrials'] as int).input(),
      maxTrials: (map['maxTrials'] as int).input(),
      params: (pulumi.Input.decodeList<GoogleCloudMlV1ParameterSpecResponse>(map['params'], (value) => GoogleCloudMlV1ParameterSpecResponse.fromMap((value as Map).cast<String, dynamic>()))).input(),
      resumePreviousJobId: (map['resumePreviousJobId'] as String).input(),
    );
  }
}
