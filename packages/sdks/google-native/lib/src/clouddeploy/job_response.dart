...
class JobResponse {
  ...
  factory JobResponse.fromMap(Map<String, dynamic> map) {
    return JobResponse(
      advanceChildRolloutJob: ((map['advanceChildRolloutJob'] as Map).cast<String, dynamic>()).input(),
      createChildRolloutJob: ((map['createChildRolloutJob'] as Map).cast<String, dynamic>()).input(),
      deployJob: ((map['deployJob'] as Map).cast<String, dynamic>()).input(),
      jobRun: (map['jobRun'] as String).input(),
      postdeployJob: (PostdeployJobResponse.fromMap((map['postdeployJob'] as Map).cast<String, dynamic>())).input(),
      predeployJob: (PredeployJobResponse.fromMap((map['predeployJob'] as Map).cast<String, dynamic>())).input(),
      skipMessage: (map['skipMessage'] as String).input(),
      state: (map['state'] as String).input(),
      verifyJob: ((map['verifyJob'] as Map).cast<String, dynamic>()).input(),
    );
  }
}
