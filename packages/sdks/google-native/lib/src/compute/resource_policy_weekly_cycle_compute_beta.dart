...
class ResourcePolicyWeeklyCycleComputeBeta {
  ...
  const ResourcePolicyWeeklyCycleComputeBeta({
    this.dayOfWeeks,
  });
  ...
  factory ResourcePolicyWeeklyCycleComputeBeta.fromMap(Map<String, dynamic> map) {
    return ResourcePolicyWeeklyCycleComputeBeta(
      dayOfWeeks: (() { final guardedValue = map['dayOfWeeks']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<ResourcePolicyWeeklyCycleDayOfWeekComputeBeta>(guardedValue, (value) => ResourcePolicyWeeklyCycleDayOfWeekComputeBeta.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
