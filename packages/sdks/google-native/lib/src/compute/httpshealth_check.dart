...
class HTTPSHealthCheck {
  ...
  final HTTPSHealthCheckPortSpecification? portSpecification;
  /// Specifies the type of proxy header to append before sending data to the backend, either NONE or PROXY_V1. The default is NONE.
  final HTTPSHealthCheckProxyHeader? proxyHeader;
  /// The request path of the HTTPS health check request. The default value is /.
  final String? requestPath;
  /// Creates a content-based HTTPS health check. In addition to the required HTTP 200 (OK) status code, you can configure the health check to pass only when the backend sends this specific ASCII response string within the first 1024 bytes of the HTTP response body. For details, see: https://cloud.google.com/load-balancing/docs/health-check-concepts#criteria-protocol-http
  final String? response;
  /// Weight report mode. used for weighted Load Balancing.
  final HTTPSHealthCheckWeightReportMode? weightReportMode;
  ...
  factory HTTPSHealthCheck.fromMap(Map<String, dynamic> map) {
    return HTTPSHealthCheck(
      ...
      portSpecification: map['portSpecification'] == null ? null : HTTPSHealthCheckPortSpecification.fromValue(map['portSpecification'] as String),
      proxyHeader: map['proxyHeader'] == null ? null : HTTPSHealthCheckProxyHeader.fromValue(map['proxyHeader'] as String),
      requestPath: map['requestPath'] == null ? null : map['requestPath'] as String,
      response: map['response'] == null ? null : map['response'] as String,
      weightReportMode: map['weightReportMode'] == null ? null : HTTPSHealthCheckWeightReportMode.fromValue(map['weightReportMode'] as String),
    );
  }
}
