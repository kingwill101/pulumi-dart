...
class EncryptionResponse {
  ...
  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      'aes128': aes128,
      'drmSystems':
          pulumi.Input.mapInputValue<DrmSystemsResponse, Map<String, dynamic>>(
            drmSystems,
            (value) => value.toMap(),
          ),
      'mpegCenc':
  ...
  factory EncryptionResponse.fromMap(Map<String, dynamic> map) {
    return EncryptionResponse(
      aes128: pulumi.Input.fromValue(
        (map['aes128']! as Map).cast<String, dynamic>(),
      ),
      drmSystems: pulumi.Input.fromValue(
        DrmSystemsResponse.fromMap(
          (map['drmSystems']! as Map).cast<String, dynamic>(),
        ),
      ),
      mpegCenc: pulumi.Input.fromValue(
        MpegCommonEncryptionResponse.fromMap(
          (map['mpegCenc']! as Map).cast<String, dynamic>(),
        ),
      ),
      sampleAes: pulumi.Input.fromValue(
        (map['sampleAes']! as Map).cast<String, dynamic>(),
      ),
      secretManagerKeySource: pulumi.Input.fromValue(
        SecretManagerSourceResponse.fromMap(
