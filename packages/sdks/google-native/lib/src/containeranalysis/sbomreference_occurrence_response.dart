...
class SBOMReferenceOccurrenceResponse {
  ...
  factory SBOMReferenceOccurrenceResponse.fromMap(Map<String, dynamic> map) {
    return SBOMReferenceOccurrenceResponse(
      payload: (SbomReferenceIntotoPayloadResponse.fromMap((map['payload'] as Map).cast<String, dynamic>())).input(),
      payloadType: (map['payloadType'] as String).input(),
      signatures: (pulumi.Input.decodeList<EnvelopeSignatureResponse>(map['signatures'], (value) => EnvelopeSignatureResponse.fromMap((value as Map).cast<String, dynamic>()))).input(),
    );
  }
}
