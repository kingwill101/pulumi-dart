...
class TaskGroupResponse {
  ...
  factory TaskGroupResponse.fromMap(Map<String, dynamic> map) {
    return TaskGroupResponse(
      ...
      parallelism: (map['parallelism'] as String).input(),
      permissiveSsh: (map['permissiveSsh'] as bool).input(),
      requireHostsFile: (map['requireHostsFile'] as bool).input(),
      schedulingPolicy: (map['schedulingPolicy'] as String).input(),
      taskCount: (map['taskCount'] as String).input(),
      taskCountPerNode: (map['taskCountPerNode'] as String).input(),
      taskEnvironments: (pulumi.Input.decodeList<EnvironmentResponse>(map['taskEnvironments'], (value) => EnvironmentResponse.fromMap((value as Map).cast<String, dynamic>()))).input(),
      taskSpec: (TaskSpecResponse.fromMap((map['taskSpec'] as Map).cast<String, dynamic>())).input(),
    );
  }
}
