import 'package:pulumi/pulumi.dart' as pulumi;
import 'google_cloud_aiplatform_v1_encryption_spec_response.dart';
import 'google_cloud_aiplatform_v1_metadata_store_metadata_store_state_response.dart';
import 'metadata_store_args.dart';

/// Initializes a MetadataStore, including allocation of resources.
/// Auto-naming is currently not supported for this resource.
class MetadataStore extends pulumi.CustomResource {
  /// Timestamp when this MetadataStore was created.
  late final pulumi.Output<String> createTime;
  /// Description of the MetadataStore.
  late final pulumi.Output<String> description;
  /// Customer-managed encryption key spec for a Metadata Store. If set, this Metadata Store and all sub-resources of this Metadata Store are secured using this key.
  late final pulumi.Output<GoogleCloudAiplatformV1EncryptionSpecResponse> encryptionSpec;
  late final pulumi.Output<String> location;
  /// The {metadatastore} portion of the resource name with the format: `projects/{project}/locations/{location}/metadataStores/{metadatastore}` If not provided, the MetadataStore's ID will be a UUID generated by the service. Must be 4-128 characters in length. Valid characters are `/a-z-/`. Must be unique across all MetadataStores in the parent Location. (Otherwise the request will fail with ALREADY_EXISTS, or PERMISSION_DENIED if the caller can't view the preexisting MetadataStore.)
  late final pulumi.Output<String?> metadataStoreId;
  /// The resource name of the MetadataStore instance.
  late final pulumi.Output<String> name;
  late final pulumi.Output<String> project;
  /// State information of the MetadataStore.
  late final pulumi.Output<GoogleCloudAiplatformV1MetadataStoreMetadataStoreStateResponse> state;
  /// Timestamp when this MetadataStore was last updated.
  late final pulumi.Output<String> updateTime;

  /// Creates a new [MetadataStore].
  /// [name] The Pulumi resource name.
  /// [args] Arguments used to configure this [MetadataStore]. {@macro pulumi_aiplatform_v1_metadata_store_args_doc}
  /// [options] Resource options controlling this resource's behavior.
  MetadataStore(
    String name, {
    MetadataStoreArgs? args,
    pulumi.CustomResourceOptions? options,
  }) : super(
          'google-native:aiplatform/v1:MetadataStore',
          name,
          pulumi.Input.mapToInputs(args?.toMap() ?? const {}),
          options ?? pulumi.CustomResourceOptions(),
        ) {
    createTime = registerOutput<String>('createTime');
    description = registerOutput<String>('description');
    encryptionSpec = registerOutput<GoogleCloudAiplatformV1EncryptionSpecResponse>('encryptionSpec', decoder: (raw) { final guardedValue = raw; if (guardedValue == null) return null; return GoogleCloudAiplatformV1EncryptionSpecResponse.fromMap((guardedValue as Map).cast<String, dynamic>()); });
    location = registerOutput<String>('location');
    metadataStoreId = registerOutput<String?>('metadataStoreId');
    this.name = registerOutput<String>('name');
    project = registerOutput<String>('project');
    state = registerOutput<GoogleCloudAiplatformV1MetadataStoreMetadataStoreStateResponse>('state', decoder: (raw) { final guardedValue = raw; if (guardedValue == null) return null; return GoogleCloudAiplatformV1MetadataStoreMetadataStoreStateResponse.fromMap((guardedValue as Map).cast<String, dynamic>()); });
    updateTime = registerOutput<String>('updateTime');
  }
}
