...
class GetApplicationApi {
  ...
  factory GetApplicationApi.fromMap(Map<String, dynamic> map) {
    return GetApplicationApi(
      knownClientApplications: ((map['knownClientApplications'] as List).cast<String>()).input(),
      mappedClaimsEnabled: (map['mappedClaimsEnabled'] as bool).input(),
      oauth2PermissionScopes: (pulumi.Input.decodeList<GetApplicationApiOauth2PermissionScope>(map['oauth2PermissionScopes'], (value) => GetApplicationApiOauth2PermissionScope.fromMap((value as Map).cast<String, dynamic>()))).input(),
      requestedAccessTokenVersion: (map['requestedAccessTokenVersion'] as int).input(),
    );
  }
}
