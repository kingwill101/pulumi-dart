...
class PodDisruptionBudgetPolicyV1 extends pulumi.CustomResource {
  ...
  PodDisruptionBudgetPolicyV1(
    ...
    spec = registerOutput<PodDisruptionBudgetSpec>(
      'spec',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return PodDisruptionBudgetSpec.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
    status = registerOutput<PodDisruptionBudgetStatus?>(
      'status',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return PodDisruptionBudgetStatus.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
