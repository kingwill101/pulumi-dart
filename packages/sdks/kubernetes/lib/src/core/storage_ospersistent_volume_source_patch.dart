...
class StorageOSPersistentVolumeSourcePatch {
  ...
  const StorageOSPersistentVolumeSourcePatch({
    this.fsType,
    this.readOnly,
    this.secretRef,
    this.volumeName,
  ...
  factory StorageOSPersistentVolumeSourcePatch.fromMap(Map<String, dynamic> map) {
    return StorageOSPersistentVolumeSourcePatch(
      ...
      secretRef: (() { final guardedValue = map['secretRef']; if (guardedValue == null) return null; return pulumi.Input.fromValue(ObjectReferencePatch.fromMap((guardedValue as Map).cast<String, dynamic>())); })(),
