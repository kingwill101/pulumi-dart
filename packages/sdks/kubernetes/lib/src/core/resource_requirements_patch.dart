...
class ResourceRequirementsPatch {
  ...
  const ResourceRequirementsPatch({
    this.claims,
    this.limits,
    this.requests,
  });
  ...
  factory ResourceRequirementsPatch.fromMap(Map<String, dynamic> map) {
    return ResourceRequirementsPatch(
      claims: (() { final guardedValue = map['claims']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<ResourceClaimPatch>(guardedValue, (value) => ResourceClaimPatch.fromMap((value as Map).cast<String, dynamic>()))); })(),
