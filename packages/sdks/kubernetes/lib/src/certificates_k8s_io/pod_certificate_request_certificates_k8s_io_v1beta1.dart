...
class PodCertificateRequestCertificatesK8sIoV1beta1
  ...
  PodCertificateRequestCertificatesK8sIoV1beta1(
    ...
    spec = registerOutput<PodCertificateRequestSpec>(
      'spec',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return PodCertificateRequestSpec.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
    status = registerOutput<PodCertificateRequestStatus?>(
      'status',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return PodCertificateRequestStatus.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
