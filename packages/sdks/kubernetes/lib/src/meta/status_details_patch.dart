...
class StatusDetailsPatch {
  ...
  factory StatusDetailsPatch.fromMap(Map<String, dynamic> map) {
    return StatusDetailsPatch(
      causes: (() { final guardedValue = map['causes']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<StatusCausePatch>(guardedValue, (value) => StatusCausePatch.fromMap((value as Map).cast<String, dynamic>()))); })(),
      ...
      retryAfterSeconds: (() { final guardedValue = map['retryAfterSeconds']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
