...
class PriorityLevelConfigurationFlowcontrolApiserverK8sIoV1
  ...
  PriorityLevelConfigurationFlowcontrolApiserverK8sIoV1(
    ...
    spec = registerOutput<PriorityLevelConfigurationSpec>(
      'spec',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return PriorityLevelConfigurationSpec.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
    status = registerOutput<PriorityLevelConfigurationStatus?>(
      'status',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return PriorityLevelConfigurationStatus.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
