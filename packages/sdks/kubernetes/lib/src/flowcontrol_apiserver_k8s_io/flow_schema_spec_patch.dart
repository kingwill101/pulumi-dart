...
import 'flow_distinguisher_method_patch.dart';
import 'policy_rules_with_subjects_patch.dart';
import 'priority_level_configuration_reference_patch.dart';

/// FlowSchemaSpec describes how the FlowSchema's specification looks like.
class FlowSchemaSpecPatch {
  /// `distinguisherMethod` defines how to compute the flow distinguisher for requests that match this schema. `nil` specifies that the distinguisher is disabled and thus will always be the empty string.
  final FlowDistinguisherMethodPatch? distinguisherMethod;
  ...
  final PriorityLevelConfigurationReferencePatch? priorityLevelConfiguration;
  /// `rules` describes which requests will match this flow schema. This FlowSchema matches a request if and only if at least one member of rules matches the request. if it is an empty slice, there will be no requests matching the FlowSchema.
  final List<PolicyRulesWithSubjectsPatch>? rules;

  /// Creates a new [FlowSchemaSpecPatch].
  ...
  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      ...
      'rules': ?rules == null ? null : pulumi.Input.encodeList<PolicyRulesWithSubjectsPatch, Map<String, dynamic>>(rules!, (value) => value.toMap()),
    };
  }

  factory FlowSchemaSpecPatch.fromMap(Map<String, dynamic> map) {
    return FlowSchemaSpecPatch(
      distinguisherMethod: map['distinguisherMethod'] == null ? null : FlowDistinguisherMethodPatch.fromMap((map['distinguisherMethod'] as Map).cast<String, dynamic>()),
      matchingPrecedence: map['matchingPrecedence'] == null ? null : map['matchingPrecedence'] as int,
      priorityLevelConfiguration: map['priorityLevelConfiguration'] == null ? null : PriorityLevelConfigurationReferencePatch.fromMap((map['priorityLevelConfiguration'] as Map).cast<String, dynamic>()),
      rules: map['rules'] == null ? null : pulumi.Input.decodeList<PolicyRulesWithSubjectsPatch>(map['rules'], (value) => PolicyRulesWithSubjectsPatch.fromMap((value as Map).cast<String, dynamic>())),
    );
  }
}
