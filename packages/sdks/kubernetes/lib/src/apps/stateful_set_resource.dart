...
class StatefulSetResource extends pulumi.CustomResource {
  ...
  StatefulSetResource(
    ...
    spec = registerOutput<StatefulSetSpecAppsV1beta1>(
      'spec',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return StatefulSetSpecAppsV1beta1.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
    status = registerOutput<StatefulSetStatusAppsV1beta1?>(
      'status',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return StatefulSetStatusAppsV1beta1.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
