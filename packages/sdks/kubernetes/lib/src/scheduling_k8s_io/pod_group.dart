...
class PodGroup {
  ...
  PodGroup({required this.name, required this.policy});

  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      'name': name,
      'policy':
          pulumi.Input.mapInputValue<PodGroupPolicy, Map<String, dynamic>>(
            policy,
            (value) => value.toMap(),
          ),
    };
  ...
  factory PodGroup.fromMap(Map<String, dynamic> map) {
    return PodGroup(
      ...
      policy: pulumi.Input.fromValue(
        PodGroupPolicy.fromMap((map['policy']! as Map).cast<String, dynamic>()),
      ),
    );
  }
}
