...
class HorizontalPodAutoscalerResource extends pulumi.CustomResource {
  ...
  HorizontalPodAutoscalerResource(
    ...
    spec = registerOutput<HorizontalPodAutoscalerSpecAutoscalingV2>(
      'spec',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return HorizontalPodAutoscalerSpecAutoscalingV2.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
    status = registerOutput<HorizontalPodAutoscalerStatusAutoscalingV2?>(
      'status',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return HorizontalPodAutoscalerStatusAutoscalingV2.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
