...
class ResourceClaimResourceK8sIoV1alpha3Resource extends pulumi.CustomResource {
  ...
  ResourceClaimResourceK8sIoV1alpha3Resource(
    ...
    spec = registerOutput<ResourceClaimSpecResourceK8sIoV1alpha3>(
      'spec',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return ResourceClaimSpecResourceK8sIoV1alpha3.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
    status = registerOutput<ResourceClaimStatusResourceK8sIoV1alpha3?>(
      'status',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return ResourceClaimStatusResourceK8sIoV1alpha3.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
