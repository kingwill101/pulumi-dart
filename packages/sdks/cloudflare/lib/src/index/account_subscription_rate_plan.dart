// ignore_for_file: unused_element, unnecessary_cast

import 'package:pulumi/pulumi.dart' as pulumi;

class AccountSubscriptionRatePlan {
  /// The currency applied to the rate plan subscription.
  final pulumi.Input<String?>? currency;
  /// Whether this rate plan is managed externally from Cloudflare.
  final pulumi.Input<bool?>? externallyManaged;
  /// The ID of the rate plan.
  final pulumi.Input<String?>? id;
  /// Whether a rate plan is enterprise-based (or newly adopted term contract).
  final pulumi.Input<bool?>? isContract;
  /// The full name of the rate plan.
  final pulumi.Input<String?>? publicName;
  /// The scope that this rate plan applies to.
  final pulumi.Input<String?>? scope;
  /// The list of sets this rate plan applies to. Returns array of strings.
  final pulumi.Input<List<String>?>? sets;

  /// Creates a new [AccountSubscriptionRatePlan].
  /// [currency] The currency applied to the rate plan subscription.
  /// [externallyManaged] Whether this rate plan is managed externally from Cloudflare.
  /// [id] The ID of the rate plan.
  /// [isContract] Whether a rate plan is enterprise-based (or newly adopted term contract).
  /// [publicName] The full name of the rate plan.
  /// [scope] The scope that this rate plan applies to.
  /// [sets] The list of sets this rate plan applies to. Returns array of strings.
  const AccountSubscriptionRatePlan({
    this.currency,
    this.externallyManaged,
    this.id,
    this.isContract,
    this.publicName,
    this.scope,
    this.sets,
  });

  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      'currency': ?currency,
      'externallyManaged': ?externallyManaged,
      'id': ?id,
      'isContract': ?isContract,
      'publicName': ?publicName,
      'scope': ?scope,
      'sets': ?sets,
    };
  }

  factory AccountSubscriptionRatePlan.fromMap(Map<String, dynamic> map) {
    return AccountSubscriptionRatePlan(
      currency: (() { final guardedValue = map['currency']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      externallyManaged: (() { final guardedValue = map['externallyManaged']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as bool); })(),
      id: (() { final guardedValue = map['id']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      isContract: (() { final guardedValue = map['isContract']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as bool); })(),
      publicName: (() { final guardedValue = map['publicName']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      scope: (() { final guardedValue = map['scope']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      sets: (() { final guardedValue = map['sets']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as List).cast<String>()); })(),
    );
  }
}
