...
class SlotSiteConfigIpRestriction {
  ...
  /// [headers] The `headers` block for this specific `ipRestriction` as defined below. The HTTP header filters are evaluated after the rule itself and both conditions must be true for the rule to apply.
  ...
  const SlotSiteConfigIpRestriction({
    this.action,
    this.headers,
    this.ipAddress,
    this.name,
  ...
  factory SlotSiteConfigIpRestriction.fromMap(Map<String, dynamic> map) {
    return SlotSiteConfigIpRestriction(
      ...
      headers: (() { final guardedValue = map['headers']; if (guardedValue == null) return null; return pulumi.Input.fromValue(SlotSiteConfigIpRestrictionHeaders.fromMap((guardedValue as Map).cast<String, dynamic>())); })(),
