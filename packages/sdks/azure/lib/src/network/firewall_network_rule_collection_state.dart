...
class FirewallNetworkRuleCollectionState {
  ...
  const FirewallNetworkRuleCollectionState({
    this.action,
  ...
  factory FirewallNetworkRuleCollectionState.fromMap(Map<String, dynamic> map) {
    return FirewallNetworkRuleCollectionState(
      ...
      rules: (() { final guardedValue = map['rules']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<FirewallNetworkRuleCollectionRule>(guardedValue, (value) => FirewallNetworkRuleCollectionRule.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
