...
class VirtualNetworkGatewayConnectionState {
  ...
  /// [bgpEnabled] If `true`, BGP (Border Gateway Protocol) is enabled for this connection. Defaults to `false`.
  ...
  /// [customBgpAddresses] A `customBgpAddresses` block which is documented below.
  ...
  /// [ipsecPolicy] A `ipsecPolicy` block which is documented below.
  ...
  /// [privateLinkFastPathEnabled] Bypass the Express Route gateway when accessing private-links. When enabled `expressRouteGatewayBypass` must be set to `true`. Defaults to `false`.
  ...
  /// [usePolicyBasedTrafficSelectors] If `true`, policy-based traffic selectors are enabled for this connection. Enabling policy-based traffic selectors requires an `ipsecPolicy` block. Defaults to `false`.
