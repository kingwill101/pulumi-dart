...
class GetApplicationGatewayBackendHttpSetting {
  ...
  factory GetApplicationGatewayBackendHttpSetting.fromMap(Map<String, dynamic> map) {
    return GetApplicationGatewayBackendHttpSetting(
      affinityCookieName: (map['affinityCookieName'] as String).input(),
      authenticationCertificates: (pulumi.Input.decodeList<GetApplicationGatewayBackendHttpSettingAuthenticationCertificate>(map['authenticationCertificates'], (value) => GetApplicationGatewayBackendHttpSettingAuthenticationCertificate.fromMap((value as Map).cast<String, dynamic>()))).input(),
      connectionDrainings: (pulumi.Input.decodeList<GetApplicationGatewayBackendHttpSettingConnectionDraining>(map['connectionDrainings'], (value) => GetApplicationGatewayBackendHttpSettingConnectionDraining.fromMap((value as Map).cast<String, dynamic>()))).input(),
      cookieBasedAffinity: (map['cookieBasedAffinity'] as String).input(),
      dedicatedBackendConnectionEnabled: (map['dedicatedBackendConnectionEnabled'] as bool).input(),
      hostName: (map['hostName'] as String).input(),
      id: (map['id'] as String).input(),
      name: (map['name'] as String).input(),
      path: (map['path'] as String).input(),
      pickHostNameFromBackendAddress: (map['pickHostNameFromBackendAddress'] as bool).input(),
      port: (map['port'] as int).input(),
      probeId: (map['probeId'] as String).input(),
      probeName: (map['probeName'] as String).input(),
      protocol: (map['protocol'] as String).input(),
      requestTimeout: (map['requestTimeout'] as int).input(),
      trustedRootCertificateNames: ((map['trustedRootCertificateNames'] as List).cast<String>()).input(),
    );
  }
}
