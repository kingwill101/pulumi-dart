...
class ManagedHardwareSecurityModuleKeyArgs {
  ...
  /// [curve] Specifies the curve to use when creating an `EC-HSM` key. Possible values are `P-256`, `P-256K`, `P-384`, and `P-521`. This field is required if `keyType` is `EC-HSM`. Changing this forces a new resource to be created.
  ...
  /// [keySize] Specifies the Size of the RSA key to create in bytes. For example, 1024 or 2048. *Note*: This field is required if `keyType` is `RSA-HSM` or `oct-HSM`. Changing this forces a new resource to be created.
  ...
  factory ManagedHardwareSecurityModuleKeyArgs.fromMap(Map<String, dynamic> map) {
    return ManagedHardwareSecurityModuleKeyArgs(
      ...
      keyOpts: pulumi.Input.fromValue((map['keyOpts'] as List).cast<String>()),
