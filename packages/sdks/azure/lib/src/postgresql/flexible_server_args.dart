...
/// {@template pulumi_postgresql_flexible_server_flexible_server_args_doc}
/// The set of arguments for FlexibleServer.
/// {@endtemplate}
/// {@macro pulumi_postgresql_flexible_server_flexible_server_args_doc}
class FlexibleServerArgs {
  ...
  /// [administratorLogin] The Administrator login for the PostgreSQL Flexible Server. Required when `createMode` is `Default` and `authentication.password_auth_enabled` is `true`.
  /// [administratorPassword] The Password associated with the `administratorLogin` for the PostgreSQL Flexible Server.
  ...
  /// [pointInTimeRestoreTimeInUtc] The point in time to restore from `sourceServerId` when `createMode` is `GeoRestore`, `PointInTimeRestore`. Changing this forces a new PostgreSQL Flexible Server to be created.
  ...
  /// [sourceServerId] The resource ID of the source PostgreSQL Flexible Server to be restored. Required when `createMode` is `GeoRestore`, `PointInTimeRestore` or `Replica`. Changing this forces a new PostgreSQL Flexible Server to be created.
  ...
  /// [storageTier] The name of storage performance tier for IOPS of the PostgreSQL Flexible Server. Possible values are `P4`, `P6`, `P10`, `P15`,`P20`, `P30`,`P40`, `P50`,`P60`, `P70` or `P80`. Default value is dependent on the `storageMb` value. Please see the `storageTier` defaults based on `storageMb` table below.
  /// [tags] A mapping of tags which should be assigned to the PostgreSQL Flexible Server.
  /// [version] The version of PostgreSQL Flexible Server to use. Possible values are `11`,`12`, `13`, `14`, `15`, `16`, `17`, and `18`. Required when `createMode` is `Default`.
  ...
  factory FlexibleServerArgs.fromMap(Map<String, dynamic> map) {
    return FlexibleServerArgs(
      ...
      administratorPasswordWoVersion: (() { final guardedValue = map['administratorPasswordWoVersion']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as num).toInt()); })(),
