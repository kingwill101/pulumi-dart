...
class KubernetesClusterDefaultNodePool {
  ...
  /// [fipsEnabled] Should the nodes in this Node Pool have Federal Information Processing Standard enabled? `temporaryNameForRotation` must be specified when changing this block.
  ...
  /// [hostEncryptionEnabled] Should the nodes in the Default Node Pool have host encryption enabled? `temporaryNameForRotation` must be specified when changing this property.
  ...
  /// [kubeletConfig] A `kubeletConfig` block as defined below. `temporaryNameForRotation` must be specified when changing this block.
  /// [kubeletDiskType] The type of disk used by kubelet. Possible values are `OS` and `Temporary`. `temporaryNameForRotation` must be specified when changing this block.
  /// [linuxOsConfig] A `linuxOsConfig` block as defined below. `temporaryNameForRotation` must be specified when changing this block.
  /// [maxCount] Optional.
  /// [maxPods] The maximum number of pods that can run on each agent. `temporaryNameForRotation` must be specified when changing this property.
  /// [minCount] Optional.
  ...
  /// [nodePublicIpEnabled] Should nodes in this Node Pool have a Public IP Address? `temporaryNameForRotation` must be specified when changing this property.
  ...
  /// [onlyCriticalAddonsEnabled] Enabling this option will taint default node pool with `CriticalAddonsOnly=true:NoSchedule` taint. `temporaryNameForRotation` must be specified when changing this property.
  ...
  /// [osDiskSizeGb] The size of the OS Disk which should be used for each agent in the Node Pool. `temporaryNameForRotation` must be specified when attempting a change.
  /// [osDiskType] The type of disk which should be used for the Operating System. Possible values are `Ephemeral` and `Managed`. Defaults to `Managed`. `temporaryNameForRotation` must be specified when attempting a change.
  /// [osSku] Specifies the OS SKU used by the agent pool. Possible values are `AzureLinux`, `AzureLinux3`, `Ubuntu`, `Ubuntu2204`, `Ubuntu2404`, `Windows2019` and `Windows2022`. If not specified, the default is `Ubuntu` when os_type=Linux or `Windows2019` if os_type=Windows (`Windows2022` Kubernetes ≥1.33). Changing between `AzureLinux` and `Ubuntu` does not replace the resource; otherwise `temporaryNameForRotation` must be specified when attempting a change.
  ...
  /// [snapshotId] The ID of the Snapshot which should be used to create this default Node Pool. `temporaryNameForRotation` must be specified when changing this property.
  ...
  /// [ultraSsdEnabled] Used to specify whether the UltraSSD is enabled in the Default Node Pool. Defaults to `false`. See [the documentation](https://docs.microsoft.com/azure/aks/use-ultra-disks) for more information. `temporaryNameForRotation` must be specified when attempting a change.
  /// [upgradeSettings] A `upgradeSettings` block as documented below.
  /// [vmSize] The size of the Virtual Machine, such as `Standard_DS2_v2`. `temporaryNameForRotation` must be specified when attempting a resize.
  /// [vnetSubnetId] The ID of a Subnet where the Kubernetes Node Pool should exist.
  /// [workloadRuntime] Specifies the workload runtime used by the node pool. Possible values are `KataVmIsolation` and `OCIContainer`.
  /// [zones] Specifies a list of Availability Zones in which this Kubernetes Cluster should be located. `temporaryNameForRotation` must be specified when changing this property.
  ...
  factory KubernetesClusterDefaultNodePool.fromMap(Map<String, dynamic> map) {
    return KubernetesClusterDefaultNodePool(
      ...
      maxPods: (() { final guardedValue = map['maxPods']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as num).toInt()); })(),
      ...
      osDiskSizeGb: (() { final guardedValue = map['osDiskSizeGb']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as num).toInt()); })(),
