...
///   connection_string = "Host=example;Port=5432;Database=example;UID=example;EncryptionMethod=0;Password=example"
/// }
/// resource "azure_datafactory_datasetpostgresql" "example" {
///   name                = "example"
///   data_factory_id     = azure_datafactory_factory.example.id
///   linked_service_name = azure_datafactory_linkedservicepostgresql.example.name
/// }
/// ```
/// ```java
...
class DatasetPostgresql extends pulumi.CustomResource {
  ...
  DatasetPostgresql(
    ...
    schemaColumns = registerOutput<List<DatasetPostgresqlSchemaColumn>?>('schemaColumns', decoder: (raw) { final guardedValue = raw; if (guardedValue == null) return null; return pulumi.Input.decodeList<DatasetPostgresqlSchemaColumn>(guardedValue, (value) => DatasetPostgresqlSchemaColumn.fromMap((value as Map).cast<String, dynamic>())); });
  ...
  /// Creates a typed reference to an existing [DatasetPostgresql] resource.
  DatasetPostgresql.reference(String urn)
    : super(
        'azure:datafactory/datasetPostgresql:DatasetPostgresql',
