# pulumi_gcp_global_cloudrun

Generated Pulumi provider SDK for Dart.

## Installation

~~~sh
dart pub add pulumi_gcp_global_cloudrun
~~~

## Usage

~~~dart
import 'package:pulumi_gcp_global_cloudrun/pulumi_gcp_global_cloudrun.dart' as provider;
~~~

## Alternative: Use directly from GitHub

~~~yaml
dependencies:
  pulumi_gcp_global_cloudrun:
    git:
      url: https://github.com/<owner>/<repo>.git
      path: packages/gcp-global-cloudrun
      ref: main
~~~

See the example/ directory for a runnable sample.

_This file is preserved across SDK regeneration done via task generate:<provider>._
