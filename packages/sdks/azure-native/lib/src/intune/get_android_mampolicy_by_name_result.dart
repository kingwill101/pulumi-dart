...
class GetAndroidMAMPolicyByNameResult {
  ...
  GetAndroidMAMPolicyByNameResult({
    ...
    this.tags,
    this.type,
  }) : appSharingFromLevel = appSharingFromLevel ?? 'none', appSharingToLevel = appSharingToLevel ?? 'none', authentication = authentication ?? 'required', clipboardSharingLevel = clipboardSharingLevel ?? 'blocked', dataBackup = dataBackup ?? 'allow', deviceCompliance = deviceCompliance ?? 'enable', fileEncryption = fileEncryption ?? 'required', fileSharingSaveAs = fileSharingSaveAs ?? 'allow', groupStatus = groupStatus ?? 'notTargeted', managedBrowser = managedBrowser ?? 'required', pin = pin ?? 'required', screenCapture = screenCapture ?? 'allow';
