...
class CircuitBreakerFailureCondition {
  ...
  const CircuitBreakerFailureCondition({
    this.count,
    this.errorReasons,
    this.interval,
    this.percentage,
  ...
  factory CircuitBreakerFailureCondition.fromMap(Map<String, dynamic> map) {
    return CircuitBreakerFailureCondition(
      ...
      statusCodeRanges: (() { final guardedValue = map['statusCodeRanges']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<FailureStatusCodeRange>(guardedValue, (value) => FailureStatusCodeRange.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
