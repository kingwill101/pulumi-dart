...
class BackendBaseParametersResponsePool {
  ...
  const BackendBaseParametersResponsePool({
    this.services,
  });
  ...
  factory BackendBaseParametersResponsePool.fromMap(Map<String, dynamic> map) {
    return BackendBaseParametersResponsePool(
      services: (() { final guardedValue = map['services']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<BackendPoolItemResponse>(guardedValue, (value) => BackendPoolItemResponse.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
