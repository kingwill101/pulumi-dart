...
class DatasetCreateRequestParameters {
  /// Header type.
  final pulumi.Input<dynamic>? header;
  ...
  DatasetCreateRequestParameters({
    this.header,
    pulumi.Input<bool?>? includePath,
    this.partitionFormat,
    this.path,
    this.query,
    this.separator,
    this.sourceType,
  }) : includePath = includePath ?? pulumi.Input.fromValue(false);
  ...
  factory DatasetCreateRequestParameters.fromMap(Map<String, dynamic> map) {
    return DatasetCreateRequestParameters(
      header: (() { final guardedValue = map['header']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue); })(),
