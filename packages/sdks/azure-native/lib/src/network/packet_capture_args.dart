...
class PacketCaptureArgs {
  ...
  PacketCaptureArgs({
    pulumi.Input<double?>? bytesToCapturePerPacket,
    ...
    this.targetType,
    pulumi.Input<int?>? timeLimitInSeconds,
    pulumi.Input<double?>? totalBytesPerSession,
  }) : bytesToCapturePerPacket = bytesToCapturePerPacket ?? pulumi.Input.fromValue(0), timeLimitInSeconds = timeLimitInSeconds ?? pulumi.Input.fromValue(18000), totalBytesPerSession = totalBytesPerSession ?? pulumi.Input.fromValue(1.073741824e+09);
  ...
  factory PacketCaptureArgs.fromMap(Map<String, dynamic> map) {
    return PacketCaptureArgs(
      bytesToCapturePerPacket: (() { final guardedValue = map['bytesToCapturePerPacket']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as num).toDouble()); })(),
      ...
      timeLimitInSeconds: (() { final guardedValue = map['timeLimitInSeconds']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      totalBytesPerSession: (() { final guardedValue = map['totalBytesPerSession']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as num).toDouble()); })(),
    );
  }
}
