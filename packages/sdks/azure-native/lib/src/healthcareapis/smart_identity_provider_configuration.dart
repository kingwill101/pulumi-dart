...
class SmartIdentityProviderConfiguration {
  ...
  const SmartIdentityProviderConfiguration({
    this.applications,
    this.authority,
  });
  ...
  factory SmartIdentityProviderConfiguration.fromMap(Map<String, dynamic> map) {
    return SmartIdentityProviderConfiguration(
      applications: (() { final guardedValue = map['applications']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<SmartIdentityProviderApplication>(guardedValue, (value) => SmartIdentityProviderApplication.fromMap((value as Map).cast<String, dynamic>()))); })(),
