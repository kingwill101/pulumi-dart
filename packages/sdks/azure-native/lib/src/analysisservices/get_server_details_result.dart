...
class GetServerDetailsResult {
  ...
  GetServerDetailsResult({
    ...
    this.tags,
    this.type,
  }) : managedMode = managedMode ?? 1, querypoolConnectionMode = querypoolConnectionMode ?? 'All', serverMonitorMode = serverMonitorMode ?? 1;
  ...
  factory GetServerDetailsResult.fromMap(Map<String, dynamic> map) {
    return GetServerDetailsResult(
      ...
      managedMode: (() { final guardedValue = map['managedMode']; if (guardedValue == null) return null; return ((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue); })(),
      ...
      serverFullName: (() { final guardedValue = map['serverFullName']; if (guardedValue == null) return null; return guardedValue as String; })(),
      serverMonitorMode: (() { final guardedValue = map['serverMonitorMode']; if (guardedValue == null) return null; return ((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue); })(),
