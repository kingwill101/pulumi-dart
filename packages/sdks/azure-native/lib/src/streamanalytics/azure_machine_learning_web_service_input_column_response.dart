...
class AzureMachineLearningWebServiceInputColumnResponse {
  ...
  const AzureMachineLearningWebServiceInputColumnResponse({
    this.dataType,
    this.mapTo,
    this.name,
  });
  ...
  factory AzureMachineLearningWebServiceInputColumnResponse.fromMap(Map<String, dynamic> map) {
    return AzureMachineLearningWebServiceInputColumnResponse(
      ...
      mapTo: (() { final guardedValue = map['mapTo']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
