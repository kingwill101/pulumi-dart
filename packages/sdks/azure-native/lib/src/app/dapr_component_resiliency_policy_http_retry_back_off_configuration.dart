...
class DaprComponentResiliencyPolicyHttpRetryBackOffConfiguration {
  ...
  const DaprComponentResiliencyPolicyHttpRetryBackOffConfiguration({
  ...
  factory DaprComponentResiliencyPolicyHttpRetryBackOffConfiguration.fromMap(Map<String, dynamic> map) {
    return DaprComponentResiliencyPolicyHttpRetryBackOffConfiguration(
      initialDelayInMilliseconds: (() { final guardedValue = map['initialDelayInMilliseconds']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      maxIntervalInMilliseconds: (() { final guardedValue = map['maxIntervalInMilliseconds']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
    );
  }
}
