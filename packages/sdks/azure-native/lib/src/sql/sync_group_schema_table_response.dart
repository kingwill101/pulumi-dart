...
class SyncGroupSchemaTableResponse {
  ...
  const SyncGroupSchemaTableResponse({
    this.columns,
    this.quotedName,
  });
  ...
  factory SyncGroupSchemaTableResponse.fromMap(Map<String, dynamic> map) {
    return SyncGroupSchemaTableResponse(
      columns: (() { final guardedValue = map['columns']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<SyncGroupSchemaTableColumnResponse>(guardedValue, (value) => SyncGroupSchemaTableColumnResponse.fromMap((value as Map).cast<String, dynamic>()))); })(),
