...
class RulePropertiesResponse {
  ...
  RulePropertiesResponse({required this.action, required this.addressList});

  Map<String, dynamic> toMap() {
    return <String, dynamic>{'action': action, 'addressList': addressList};
  }

  factory RulePropertiesResponse.fromMap(Map<String, dynamic> map) {
    return RulePropertiesResponse(
      action: pulumi.Input.fromValue(map['action'] as String),
      addressList: pulumi.Input.fromValue(
        (map['addressList'] as List).cast<String>(),
      ),
    );
  }
}
