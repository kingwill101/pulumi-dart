...
class CustomErrorResponseResponse {
  ...
  /// Creates a new [CustomErrorResponseResponse].
  ...
  CustomErrorResponseResponse({
    pulumi.Input<int?>? errorCachingMinTTL,
    this.errorCode,
    this.responseCode,
    this.responsePagePath,
  }) : errorCachingMinTTL = errorCachingMinTTL ?? pulumi.Input.fromValue(300);
  ...
  factory CustomErrorResponseResponse.fromMap(Map<String, dynamic> map) {
    return CustomErrorResponseResponse(
      ...
      errorCode: (() { final guardedValue = map['errorCode']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
