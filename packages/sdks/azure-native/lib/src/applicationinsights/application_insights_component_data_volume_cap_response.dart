...
class ApplicationInsightsComponentDataVolumeCapResponse {
  ...
  factory ApplicationInsightsComponentDataVolumeCapResponse.fromMap(Map<String, dynamic> map) {
    return ApplicationInsightsComponentDataVolumeCapResponse(
      ...
      maxHistoryCap: pulumi.Input.fromValue((map['maxHistoryCap'] as num).toDouble()),
      resetTime: pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(map['resetTime'])),
