...
class AutoscaleRecurrenceResponse {
  ...
  const AutoscaleRecurrenceResponse({
    this.schedule,
    this.timeZone,
  });
  ...
  factory AutoscaleRecurrenceResponse.fromMap(Map<String, dynamic> map) {
    return AutoscaleRecurrenceResponse(
      schedule: (() { final guardedValue = map['schedule']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<AutoscaleScheduleResponse>(guardedValue, (value) => AutoscaleScheduleResponse.fromMap((value as Map).cast<String, dynamic>()))); })(),
