...
class DomainDevicesRedirDevSourcePipe {
  ...
  const DomainDevicesRedirDevSourcePipe({
    required this.path,
    this.secLabels,
  });
  ...
  factory DomainDevicesRedirDevSourcePipe.fromMap(Map<String, dynamic> map) {
    return DomainDevicesRedirDevSourcePipe(
      ...
      secLabels: (() { final guardedValue = map['secLabels']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<DomainDevicesRedirDevSourcePipeSecLabel>(guardedValue, (value) => DomainDevicesRedirDevSourcePipeSecLabel.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
