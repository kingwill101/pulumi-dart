...
class DomainDevicesRedirDevSourceDev {
  ...
  const DomainDevicesRedirDevSourceDev({
    required this.path,
    this.secLabels,
  });
  ...
  factory DomainDevicesRedirDevSourceDev.fromMap(Map<String, dynamic> map) {
    return DomainDevicesRedirDevSourceDev(
      ...
      secLabels: (() { final guardedValue = map['secLabels']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<DomainDevicesRedirDevSourceDevSecLabel>(guardedValue, (value) => DomainDevicesRedirDevSourceDevSecLabel.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
