...
class DomainDevicesDiskEncryption {
  ...
  const DomainDevicesDiskEncryption({
    this.engine,
    this.format,
    this.secrets,
  });
  ...
  factory DomainDevicesDiskEncryption.fromMap(Map<String, dynamic> map) {
    return DomainDevicesDiskEncryption(
      ...
      secrets: (() { final guardedValue = map['secrets']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<DomainDevicesDiskEncryptionSecret>(guardedValue, (value) => DomainDevicesDiskEncryptionSecret.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
