...
class DomainDevicesDiskSourceEncryption {
  ...
  const DomainDevicesDiskSourceEncryption({
    this.engine,
    this.format,
    this.secrets,
  });
  ...
  factory DomainDevicesDiskSourceEncryption.fromMap(Map<String, dynamic> map) {
    return DomainDevicesDiskSourceEncryption(
      ...
      secrets: (() { final guardedValue = map['secrets']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<DomainDevicesDiskSourceEncryptionSecret>(guardedValue, (value) => DomainDevicesDiskSourceEncryptionSecret.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
