...
class DomainDevicesConsoleSourceFile {
  ...
  const DomainDevicesConsoleSourceFile({
    this.append,
    required this.path,
    this.secLabels,
  });
  ...
  factory DomainDevicesConsoleSourceFile.fromMap(Map<String, dynamic> map) {
    return DomainDevicesConsoleSourceFile(
      ...
      secLabels: (() { final guardedValue = map['secLabels']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<DomainDevicesConsoleSourceFileSecLabel>(guardedValue, (value) => DomainDevicesConsoleSourceFileSecLabel.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
