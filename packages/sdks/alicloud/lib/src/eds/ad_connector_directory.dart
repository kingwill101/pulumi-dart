...
import 'ad_connector_directory_state.dart';
...
class AdConnectorDirectory extends pulumi.CustomResource {
  ...
  AdConnectorDirectory(
    ...
    subDomainDnsAddresses = registerOutput<List<String>?>('subDomainDnsAddresses');
    subDomainName = registerOutput<String?>('subDomainName');
    vswitchIds = registerOutput<List<String>>('vswitchIds');
  }

  /// Gets an existing [AdConnectorDirectory] resource's state with the given [name] and [id].
  static AdConnectorDirectory get(
    String name,
    pulumi.Input<String> id, {
    AdConnectorDirectoryState? state,
  }) {
    return AdConnectorDirectory._get(
      name,
      state: state?.toMap(),
