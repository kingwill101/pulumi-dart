...
class MasterSlaveServerGroupState {
  ...
  const MasterSlaveServerGroupState({
  ...
  factory MasterSlaveServerGroupState.fromMap(Map<String, dynamic> map) {
    return MasterSlaveServerGroupState(
      ...
      servers: (() { final guardedValue = map['servers']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<MasterSlaveServerGroupServer>(guardedValue, (value) => MasterSlaveServerGroupServer.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
