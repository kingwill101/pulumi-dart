...
import 'security_preference_state.dart';
...
class SecurityPreference extends pulumi.CustomResource {
  ...
  SecurityPreference(
    ...
    allowUserToChangePassword = registerOutput<bool>('allowUserToChangePassword');
    allowUserToLoginWithPasskey = registerOutput<bool>('allowUserToLoginWithPasskey');
    allowUserToManageAccessKeys = registerOutput<bool>('allowUserToManageAccessKeys');
    allowUserToManageMfaDevices = registerOutput<bool>('allowUserToManageMfaDevices');
    allowUserToManagePersonalDingTalk = registerOutput<bool>('allowUserToManagePersonalDingTalk');
    enableSaveMfaTicket = registerOutput<bool>('enableSaveMfaTicket');
    enforceMfaForLogin = registerOutput<bool>('enforceMfaForLogin');
    loginNetworkMasks = registerOutput<String?>('loginNetworkMasks');
    loginSessionDuration = registerOutput<int>('loginSessionDuration');
    maxIdleDaysForAccessKeys = registerOutput<int>('maxIdleDaysForAccessKeys');
    maxIdleDaysForUsers = registerOutput<int>('maxIdleDaysForUsers');
    mfaOperationForLogin = registerOutput<String>('mfaOperationForLogin');
    operationForRiskLogin = registerOutput<String>('operationForRiskLogin');
    verificationTypes = registerOutput<List<String>?>('verificationTypes');
  }

  /// Gets an existing [SecurityPreference] resource's state with the given [name] and [id].
  static SecurityPreference get(
    String name,
    pulumi.Input<String> id, {
    SecurityPreferenceState? state,
  }) {
    return SecurityPreference._get(
      name,
      state: state?.toMap(),
