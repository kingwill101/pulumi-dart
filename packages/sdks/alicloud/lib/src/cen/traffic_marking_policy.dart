...
import 'traffic_marking_policy_state.dart';
...
class TrafficMarkingPolicy extends pulumi.CustomResource {
  ...
  TrafficMarkingPolicy(
    ...
    markingDscp = registerOutput<int>('markingDscp');
  ...
  /// Gets an existing [TrafficMarkingPolicy] resource's state with the given [name] and [id].
  static TrafficMarkingPolicy get(
    String name,
    pulumi.Input<String> id, {
    TrafficMarkingPolicyState? state,
  }) {
    return TrafficMarkingPolicy._get(
      name,
      state: state?.toMap(),
