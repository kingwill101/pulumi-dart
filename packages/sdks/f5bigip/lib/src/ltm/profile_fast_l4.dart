...
import 'profile_fast_l4_state.dart';
...
class ProfileFastL4 extends pulumi.CustomResource {
  ...
  ProfileFastL4(
    ...
    hardwareSyncookie = registerOutput<String>('hardwareSyncookie');
    idleTimeout = registerOutput<String>('idleTimeout');
    iptosToclient = registerOutput<String>('iptosToclient');
    iptosToserver = registerOutput<String>('iptosToserver');
    keepaliveInterval = registerOutput<String>('keepaliveInterval');
    lateBinding = registerOutput<String>('lateBinding');
    looseClose = registerOutput<String>('looseClose');
    looseInitiation = registerOutput<String>('looseInitiation');
    this.name = registerOutput<String>('name');
    partition = registerOutput<String>('partition');
    receiveWindowsize = registerOutput<int>('receiveWindowsize');
    tcpHandshakeTimeout = registerOutput<String>('tcpHandshakeTimeout');
  }

  /// Gets an existing [ProfileFastL4] resource's state with the given [name] and [id].
  static ProfileFastL4 get(
    String name,
    pulumi.Input<String> id, {
    ProfileFastL4State? state,
  }) {
    return ProfileFastL4._get(
      name,
      state: state?.toMap(),
