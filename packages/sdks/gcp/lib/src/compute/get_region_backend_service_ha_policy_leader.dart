...
class GetRegionBackendServiceHaPolicyLeader {
  ...
  factory GetRegionBackendServiceHaPolicyLeader.fromMap(Map<String, dynamic> map) {
    return GetRegionBackendServiceHaPolicyLeader(
      backendGroup: (map['backendGroup'] as String).input(),
      networkEndpoints: (pulumi.Input.decodeList<GetRegionBackendServiceHaPolicyLeaderNetworkEndpoint>(map['networkEndpoints'], (value) => GetRegionBackendServiceHaPolicyLeaderNetworkEndpoint.fromMap((value as Map).cast<String, dynamic>()))).input(),
    );
  }
}
