...
class ClusterNodePoolAutoConfig {
  ...
  /// [linuxNodeConfig] Linux system configuration for the cluster's automatically provisioned node pools. Only `cgroupMode` and `nodeKernelModuleLoading` fields are supported in `nodePoolAutoConfig`. Structure is documented below.
  /// [networkTags] The network tag config for the cluster's automatically provisioned node pools. Structure is documented below.
  /// [nodeKubeletConfig] Kubelet configuration for Autopilot clusters. Currently, only `insecureKubeletReadonlyPortEnabled` is supported here.
