...
import 'azure_node_pool_state.dart';
...
class AzureNodePool extends pulumi.CustomResource {
  ...
  /// [args] Arguments used to configure this [AzureNodePool]. {@macro pulumi_container_azure_node_pool_azure_node_pool_args_doc}
  ...
  AzureNodePool(
    ...
    autoscaling = registerOutput<AzureNodePoolAutoscaling>(
      'autoscaling',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return AzureNodePoolAutoscaling.fromMap(
    ...
    config = registerOutput<AzureNodePoolConfig>(
      'config',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return AzureNodePoolConfig.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
    ...
    management = registerOutput<AzureNodePoolManagement>(
      'management',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return AzureNodePoolManagement.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
    maxPodsConstraint = registerOutput<AzureNodePoolMaxPodsConstraint>(
      'maxPodsConstraint',
      decoder: (raw) {
        final guardedValue = raw;
        if (guardedValue == null) return null;
        return AzureNodePoolMaxPodsConstraint.fromMap(
          (guardedValue as Map).cast<String, dynamic>(),
        );
      },
    );
  ...
  /// Gets an existing [AzureNodePool] resource's state with the given [name] and [id].
  static AzureNodePool get(
    String name,
