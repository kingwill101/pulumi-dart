...
class TransferJobSchedule {
  ...
  /// [scheduleEndDate] The last day the recurring transfer will be run. If `scheduleEndDate` is the same as `scheduleStartDate`, the transfer will be executed only once. Structure documented below.
  /// [scheduleStartDate] The first day the recurring transfer is scheduled to run. If `scheduleStartDate` is in the past, the transfer will run for the first time on the following day. Structure documented below.
  /// [startTimeOfDay] The time in UTC at which the transfer will be scheduled to start in a day. Transfers may start later than this time. If not specified, recurring and one-time transfers that are scheduled to run today will run immediately; recurring transfers that are scheduled to run on a future date will start at approximately midnight UTC on that date. Note that when configuring a transfer with the Cloud Platform Console, the transfer's start time in a day is specified in your local timezone. Structure documented below.
  const TransferJobSchedule({
    this.repeatInterval,
  ...
  factory TransferJobSchedule.fromMap(Map<String, dynamic> map) {
    return TransferJobSchedule(
      ...
      scheduleEndDate: (() { final guardedValue = map['scheduleEndDate']; if (guardedValue == null) return null; return pulumi.Input.fromValue(TransferJobScheduleScheduleEndDate.fromMap((guardedValue as Map).cast<String, dynamic>())); })(),
      scheduleStartDate: pulumi.Input.fromValue(TransferJobScheduleScheduleStartDate.fromMap((map['scheduleStartDate']! as Map).cast<String, dynamic>())),
      startTimeOfDay: (() { final guardedValue = map['startTimeOfDay']; if (guardedValue == null) return null; return pulumi.Input.fromValue(TransferJobScheduleStartTimeOfDay.fromMap((guardedValue as Map).cast<String, dynamic>())); })(),
    );
  }
}
