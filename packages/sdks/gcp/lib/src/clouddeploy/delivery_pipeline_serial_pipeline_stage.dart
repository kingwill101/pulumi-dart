...
class DeliveryPipelineSerialPipelineStage {
  ...
  /// [targetId] The targetId to which this stage points. This field refers exclusively to the last segment of a target name. For example, this field would just be `my-target` (rather than `projects/project/locations/location/targets/my-target`). The location of the `Target` is inferred to be the same as the location of the `DeliveryPipeline` that contains this `Stage`.
  const DeliveryPipelineSerialPipelineStage({
  ...
  factory DeliveryPipelineSerialPipelineStage.fromMap(Map<String, dynamic> map) {
    return DeliveryPipelineSerialPipelineStage(
      deployParameters: (() { final guardedValue = map['deployParameters']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<DeliveryPipelineSerialPipelineStageDeployParameter>(guardedValue, (value) => DeliveryPipelineSerialPipelineStageDeployParameter.fromMap((value as Map).cast<String, dynamic>()))); })(),
      ...
      strategy: (() { final guardedValue = map['strategy']; if (guardedValue == null) return null; return pulumi.Input.fromValue(DeliveryPipelineSerialPipelineStageStrategy.fromMap((guardedValue as Map).cast<String, dynamic>())); })(),
