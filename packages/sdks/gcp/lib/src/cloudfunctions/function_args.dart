...
class FunctionArgs {
  ...
  /// [eventTrigger] A source that fires events in response to a condition in another service. Structure is documented below. Cannot be used with `triggerHttp`.
  ...
  /// [onDeployUpdatePolicy] Security patches are only applied when a function is redeployed. This should be specified as an empty block and cannot be set alongside `automaticUpdatePolicy`. Structure is documented below.
  ...
  /// [triggerHttp] Boolean variable. Any HTTP request (of a supported type) to the endpoint will trigger function execution. Supported HTTP request types are: POST, PUT, GET, DELETE, and OPTIONS. Endpoint is returned as `httpsTriggerUrl`. Cannot be used with `eventTrigger`.
  ...
  factory FunctionArgs.fromMap(Map<String, dynamic> map) {
    return FunctionArgs(
      ...
      dockerRegistry: (() { final guardedValue = map['dockerRegistry']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      ...
      httpsTriggerSecurityLevel: (() { final guardedValue = map['httpsTriggerSecurityLevel']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      httpsTriggerUrl: (() { final guardedValue = map['httpsTriggerUrl']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      ...
      secretEnvironmentVariables: (() { final guardedValue = map['secretEnvironmentVariables']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<FunctionSecretEnvironmentVariable>(guardedValue, (value) => FunctionSecretEnvironmentVariable.fromMap((value as Map).cast<String, dynamic>()))); })(),
      secretVolumes: (() { final guardedValue = map['secretVolumes']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<FunctionSecretVolume>(guardedValue, (value) => FunctionSecretVolume.fromMap((value as Map).cast<String, dynamic>()))); })(),
