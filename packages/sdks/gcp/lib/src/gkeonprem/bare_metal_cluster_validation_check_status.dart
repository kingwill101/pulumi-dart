...
class BareMetalClusterValidationCheckStatus {
  ...
  /// [results] (Output)
  const BareMetalClusterValidationCheckStatus({
    this.results,
  });
  ...
  factory BareMetalClusterValidationCheckStatus.fromMap(Map<String, dynamic> map) {
    return BareMetalClusterValidationCheckStatus(
      results: (() { final guardedValue = map['results']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<BareMetalClusterValidationCheckStatusResult>(guardedValue, (value) => BareMetalClusterValidationCheckStatusResult.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
