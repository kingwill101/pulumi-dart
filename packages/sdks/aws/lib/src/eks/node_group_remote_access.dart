...
class NodeGroupRemoteAccess {
  ...
  /// [sourceSecurityGroupIds] Set of EC2 Security Group IDs to allow SSH access (port 22) from on the worker nodes. If you specify `ec2SshKey`, but do not specify this configuration when you create an EKS Node Group, port 22 on the worker nodes is opened to the Internet (0.0.0.0/0).
  const NodeGroupRemoteAccess({
    this.ec2SshKey,
  ...
  factory NodeGroupRemoteAccess.fromMap(Map<String, dynamic> map) {
    return NodeGroupRemoteAccess(
      ...
      sourceSecurityGroupIds: (() { final guardedValue = map['sourceSecurityGroupIds']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as List).cast<String>()); })(),
    );
  }
}
