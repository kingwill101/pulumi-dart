...
class OptInResourceData {
  ...
  /// [dataCellsFilters] Data cell filter. See `dataCellsFilter` Block for more details.
  /// [dataLocations] Location of an Amazon S3 path where permissions are granted or revoked. See `dataLocation` Block for more details.
  /// [database] Database for the resource. Unique to the Data Catalog. A database is a set of associated table definitions organized into a logical group. You can Grant and Revoke database permissions to a principal. See `database` Block for more details.
  /// [lfTag] LF-tag key and values attached to a resource.
  /// [lfTagExpressions] Logical expression composed of one or more LF-Tag key:value pairs. See `lfTagExpression` Block for more details.
  /// [lfTagPolicies] List of LF-Tag conditions or saved LF-Tag expressions that define a resource's LF-Tag policy. See `lfTagPolicy` Block for more details.
  /// [table] Table for the resource. A table is a metadata definition that represents your data. You can Grant and Revoke table privileges to a principal. See `table` Block for more details.
  /// [tableWithColumns] Table with columns for the resource. A principal with permissions to this resource can select metadata from the columns of a table in the Data Catalog and the underlying data in Amazon S3. See `tableWithColumns` Block for more details.
