...
class GetServiceNetworkResult {
  ...
  factory GetServiceNetworkResult.fromMap(Map<String, dynamic> map) {
    return GetServiceNetworkResult(
      ...
      numberOfAssociatedServices: (() { final guardedValue = map['numberOfAssociatedServices']; if (guardedValue == null) return null; return ((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue); })(),
      numberOfAssociatedVpcs: (() { final guardedValue = map['numberOfAssociatedVpcs']; if (guardedValue == null) return null; return ((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue); })(),
