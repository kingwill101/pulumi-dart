...
class WindowsFileSystemSelfManagedActiveDirectory {
  ...
  /// [dnsIps] A list of up to two IP addresses of DNS servers or domain controllers in the self-managed AD directory. The IP addresses need to be either in the same VPC CIDR range as the file system or in the private IP version 4 (IPv4) address ranges as specified in [RFC 1918](https://tools.ietf.org/html/rfc1918).
  /// [domainJoinServiceAccountSecret] The Amazon Resource Name (ARN) for the AWS Secrets Manager secret that contains the credentials for the service account on your self-managed AD domain. Conflicts with `username` and `password`.
  /// [domainName] The fully qualified domain name of the self-managed AD directory. For example, `corp.example.com`.
  /// [fileSystemAdministratorsGroup] The name of the domain group whose members are granted administrative privileges for the file system. Administrative privileges include taking ownership of files and folders, and setting audit controls (audit ACLs) on files and folders. The group that you specify must already exist in your domain. Defaults to `Domain Admins`.
  ...
  /// [password] The password for the service account on your self-managed AD domain that Amazon FSx will use to join to your AD domain. Conflicts with `domain_join_service_account_secret`.
  /// [username] The user name for the service account on your self-managed AD domain that Amazon FSx will use to join to your AD domain. Conflicts with `domain_join_service_account_secret`.
  ...
  factory WindowsFileSystemSelfManagedActiveDirectory.fromMap(
    ...
    return WindowsFileSystemSelfManagedActiveDirectory(
      ...
      domainJoinServiceAccountSecret: (() {
        final guardedValue = map['domainJoinServiceAccountSecret'];
