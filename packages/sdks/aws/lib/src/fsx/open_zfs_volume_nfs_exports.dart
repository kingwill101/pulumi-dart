...
class OpenZfsVolumeNfsExports {
  ...
  /// [clientConfigurations] A list of configuration objects that contain the client and options for mounting the OpenZFS file system. Maximum of 25 items. See `clientConfigurations` Block below for details.
  const OpenZfsVolumeNfsExports({
  ...
  factory OpenZfsVolumeNfsExports.fromMap(Map<String, dynamic> map) {
    return OpenZfsVolumeNfsExports(
      clientConfigurations: pulumi.Input.fromValue(pulumi.Input.decodeList<OpenZfsVolumeNfsExportsClientConfiguration>(map['clientConfigurations']!, (value) => OpenZfsVolumeNfsExportsClientConfiguration.fromMap((value as Map).cast<String, dynamic>()))),
    );
  }
}
