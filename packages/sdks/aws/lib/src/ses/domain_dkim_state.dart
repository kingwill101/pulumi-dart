// ignore_for_file: unused_element, unnecessary_cast

import 'package:pulumi/pulumi.dart' as pulumi;

/// Input properties used for looking up and filtering DomainDkim resources.
class DomainDkimState {
  /// DKIM tokens generated by SES.
  /// These tokens should be used to create CNAME records used to verify SES Easy DKIM.
  /// See below for an example of how this might be achieved
  /// when the domain is hosted in Route 53 and managed by this provider.
  /// Find out more about verifying domains in Amazon SES
  /// in the [AWS SES docs](http://docs.aws.amazon.com/ses/latest/DeveloperGuide/easy-dkim-dns-records.html).
  final pulumi.Input<List<String>>? dkimTokens;
  /// Verified domain name to generate DKIM tokens for.
  final pulumi.Input<String>? domain;
  /// Region where this resource will be [managed](https://docs.aws.amazon.com/general/latest/gr/rande.html#regional-endpoints). Defaults to the Region set in the provider configuration.
  final pulumi.Input<String>? region;

  /// Creates a new [DomainDkimState].
  /// [dkimTokens] DKIM tokens generated by SES.
  /// [domain] Verified domain name to generate DKIM tokens for.
  /// [region] Region where this resource will be [managed](https://docs.aws.amazon.com/general/latest/gr/rande.html#regional-endpoints). Defaults to the Region set in the provider configuration.
  const DomainDkimState({
    this.dkimTokens,
    this.domain,
    this.region,
  });

  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      'dkimTokens': ?dkimTokens,
      'domain': ?domain,
      'region': ?region,
    };
  }

  factory DomainDkimState.fromMap(Map<String, dynamic> map) {
    return DomainDkimState(
      dkimTokens: (() { final guardedValue = map['dkimTokens']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as List).cast<String>()); })(),
      domain: (() { final guardedValue = map['domain']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      region: (() { final guardedValue = map['region']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
    );
  }
}
