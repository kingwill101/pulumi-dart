...
class CachesIscsiVolumeArgs {
  ...
  /// [kmsKey] ARN of the AWS KMS key used for Amazon S3 server side encryption. Is required when `kmsEncrypted` is set.
  ...
  factory CachesIscsiVolumeArgs.fromMap(Map<String, dynamic> map) {
    return CachesIscsiVolumeArgs(
      ...
      volumeSizeInBytes: pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(map['volumeSizeInBytes'])),
    );
  }
}
