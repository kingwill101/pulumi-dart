...
class EventSourceMappingScalingConfig {
  ...
  const EventSourceMappingScalingConfig({
  ...
  factory EventSourceMappingScalingConfig.fromMap(Map<String, dynamic> map) {
    return EventSourceMappingScalingConfig(
      maximumConcurrency: (() { final guardedValue = map['maximumConcurrency']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
    );
  }
}
