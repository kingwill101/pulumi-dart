...
class AccessKeyState {
  ...
  /// [encryptedSecret] Encrypted secret, base64 encoded, if `pgpKey` was specified. This attribute is not available for imported resources. The encrypted secret may be decrypted using the command line.
  /// [encryptedSesSmtpPasswordV4] Encrypted SES SMTP password, base64 encoded, if `pgpKey` was specified. This attribute is not available for imported resources. The encrypted password may be decrypted using the command line.
  ...
  /// [pgpKey] Either a base-64 encoded PGP public key, or a keybase username in the form `keybase:some_person_that_exists`, for use in the `encryptedSecret` output attribute. If providing a base-64 encoded PGP public key, make sure to provide the "raw" version and not the "armored" one (e.g. avoid passing the `-a` option to `gpg --export`).
  ...
  factory AccessKeyState.fromMap(Map<String, dynamic> map) {
    return AccessKeyState(
      ...
      encryptedSesSmtpPasswordV4: (() { final guardedValue = map['encryptedSesSmtpPasswordV4']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      ...
      sesSmtpPasswordV4: (() { final guardedValue = map['sesSmtpPasswordV4']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
