...
class GetCoreNetworkPolicyDocumentAttachmentPolicyAction {
  ...
  /// [segment] Name of the `segment` to share as defined in the `segments` section. This is used only when the `associationMethod` is `constant`.
  /// [tagValueOfKey] Maps the attachment to the value of a known key. This is used with the `associationMethod` is `tag`. For example a `tag` of `stage = “test”`, will map to a segment named `test`. The value must exactly match the name of a segment. This allows you to have many segments, but use only a single rule without having to define multiple nearly identical conditions. This prevents creating many similar conditions that all use the same keys to map to segments.
  const GetCoreNetworkPolicyDocumentAttachmentPolicyAction({
  ...
  factory GetCoreNetworkPolicyDocumentAttachmentPolicyAction.fromMap(Map<String, dynamic> map) {
    return GetCoreNetworkPolicyDocumentAttachmentPolicyAction(
      addToNetworkFunctionGroup: (() { final guardedValue = map['addToNetworkFunctionGroup']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      associationMethod: (() { final guardedValue = map['associationMethod']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      requireAcceptance: (() { final guardedValue = map['requireAcceptance']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as bool); })(),
      segment: (() { final guardedValue = map['segment']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
      tagValueOfKey: (() { final guardedValue = map['tagValueOfKey']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue as String); })(),
    );
  }
}
