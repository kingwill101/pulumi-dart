...
class HealthCheckState {
  ...
  /// [routingControlArn] ARN for the Route 53 Application Recovery Controller routing control. This is used when health check type is `RECOVERY_CONTROL`
  ...
  factory HealthCheckState.fromMap(Map<String, dynamic> map) {
    return HealthCheckState(
      ...
      childHealthThreshold: (() { final guardedValue = map['childHealthThreshold']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      ...
      requestInterval: (() { final guardedValue = map['requestInterval']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
