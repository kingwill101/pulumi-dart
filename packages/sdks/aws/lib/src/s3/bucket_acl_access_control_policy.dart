...
class BucketAclAccessControlPolicy {
  ...
  const BucketAclAccessControlPolicy({
    this.grants,
    required this.owner,
  });
  ...
  factory BucketAclAccessControlPolicy.fromMap(Map<String, dynamic> map) {
    return BucketAclAccessControlPolicy(
      grants: (() { final guardedValue = map['grants']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<BucketAclAccessControlPolicyGrant>(guardedValue, (value) => BucketAclAccessControlPolicyGrant.fromMap((value as Map).cast<String, dynamic>()))); })(),
      owner: pulumi.Input.fromValue(BucketAclAccessControlPolicyOwner.fromMap((map['owner']! as Map).cast<String, dynamic>())),
    );
  }
}
