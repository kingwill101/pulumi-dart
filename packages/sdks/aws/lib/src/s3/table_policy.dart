...
import 'table_policy_state.dart';
...
class TablePolicy extends pulumi.CustomResource {
  ...
  /// Gets an existing [TablePolicy] resource's state with the given [name] and [id].
  static TablePolicy get(
    String name,
    pulumi.Input<String> id, {
    TablePolicyState? state,
    pulumi.CustomResourceOptions? options,
  }) {
    return TablePolicy._get(
      name,
      state: state?.toMap(),
  ...
  /// Creates a typed reference to an existing [TablePolicy] resource.
  TablePolicy.reference(String urn)
    : super(
        'aws:s3tables/tablePolicy:TablePolicy',
