...
class DirectoryBucketAccessPointScopeState {
  ...
  const DirectoryBucketAccessPointScopeState({
    this.accountId,
    this.name,
    this.region,
    this.scope,
  ...
  factory DirectoryBucketAccessPointScopeState.fromMap(Map<String, dynamic> map) {
    return DirectoryBucketAccessPointScopeState(
      ...
      scope: (() { final guardedValue = map['scope']; if (guardedValue == null) return null; return pulumi.Input.fromValue(DirectoryBucketAccessPointScopeScope.fromMap((guardedValue as Map).cast<String, dynamic>())); })(),
    );
  }
}
