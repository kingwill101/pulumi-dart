...
class ReplicationConfigComputeConfig {
  ...
  factory ReplicationConfigComputeConfig.fromMap(Map<String, dynamic> map) {
    return ReplicationConfigComputeConfig(
      ...
      maxCapacityUnits: (() { final guardedValue = map['maxCapacityUnits']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      minCapacityUnits: (() { final guardedValue = map['minCapacityUnits']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
