...
class KxClusterAutoScalingConfiguration {
  ...
  factory KxClusterAutoScalingConfiguration.fromMap(Map<String, dynamic> map) {
    return KxClusterAutoScalingConfiguration(
      ...
      metricTarget: (map['metricTarget'] as double).input(),
      minNodeCount: (map['minNodeCount'] as int).input(),
      scaleInCooldownSeconds: (map['scaleInCooldownSeconds'] as double).input(),
      scaleOutCooldownSeconds: (map['scaleOutCooldownSeconds'] as double).input(),
    );
  }
}
