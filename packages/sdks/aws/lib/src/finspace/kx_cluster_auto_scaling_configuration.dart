...
class KxClusterAutoScalingConfiguration {
  ...
  /// [metricTarget] Desired value of chosen `autoScalingMetric`. When metric drops below this value, cluster will scale in. When metric goes above this value, cluster will scale out. Can be set between 0 and 100 percent.
  /// [minNodeCount] Lowest number of nodes to scale. Must be at least 1 and less than the `maxNodeCount`. If nodes in cluster belong to multiple availability zones, then `minNodeCount` must be at least 3.
  ...
  factory KxClusterAutoScalingConfiguration.fromMap(Map<String, dynamic> map) {
    return KxClusterAutoScalingConfiguration(
      ...
      scaleInCooldownSeconds: pulumi.Input.fromValue(map['scaleInCooldownSeconds'] as double),
      scaleOutCooldownSeconds: pulumi.Input.fromValue(map['scaleOutCooldownSeconds'] as double),
    );
  }
}
