...
class ManagedUserPoolClientRefreshTokenRotation {
  ...
  const ManagedUserPoolClientRefreshTokenRotation({
  ...
  factory ManagedUserPoolClientRefreshTokenRotation.fromMap(Map<String, dynamic> map) {
    return ManagedUserPoolClientRefreshTokenRotation(
      ...
      retryGracePeriodSeconds: (() { final guardedValue = map['retryGracePeriodSeconds']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
    );
  }
}
