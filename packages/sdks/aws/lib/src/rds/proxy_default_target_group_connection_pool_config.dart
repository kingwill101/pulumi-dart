...
class ProxyDefaultTargetGroupConnectionPoolConfig {
  ...
  /// [connectionBorrowTimeout] Number of seconds for a proxy to wait for a connection to become available in the connection pool. Only applies when the proxy has opened its maximum number of connections and all connections are busy with client sessions.
  ...
  /// [maxConnectionsPercent] Maximum size of the connection pool for each target in a target group. For Aurora MySQL, it is expressed as a percentage of the maxConnections setting for the RDS DB instance or Aurora DB cluster used by the target group.
  ...
  factory ProxyDefaultTargetGroupConnectionPoolConfig.fromMap(Map<String, dynamic> map) {
    return ProxyDefaultTargetGroupConnectionPoolConfig(
      connectionBorrowTimeout: (() { final guardedValue = map['connectionBorrowTimeout']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      ...
      maxConnectionsPercent: (() { final guardedValue = map['maxConnectionsPercent']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      maxIdleConnectionsPercent: (() { final guardedValue = map['maxIdleConnectionsPercent']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
