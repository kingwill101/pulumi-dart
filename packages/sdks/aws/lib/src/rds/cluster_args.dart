...
class ClusterArgs {
  ...
  final pulumi.Input<dynamic> engine;
  ...
  final pulumi.Input<dynamic>? engineMode;
  ...
  /// [allocatedStorage] Amount of storage in gibibytes (GiB) to allocate to each DB instance in the Multi-AZ DB cluster.
  /// [allowMajorVersionUpgrade] Enable to allow major engine version upgrades when changing engine versions. Defaults to `false`.
  /// [applyImmediately] Whether any cluster modifications are applied immediately, or during the next maintenance window. Default is `false`. See [Amazon RDS Documentation for more information.](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Overview.DBInstance.Modifying.html)
  ...
  /// [caCertificateIdentifier] CA certificate identifier to use for the DB cluster's server certificate.
  /// [clusterIdentifier] Cluster identifier. If omitted, this provider will assign a random, unique identifier.
  ...
  /// [clusterScalabilityType] Scalability mode of the Aurora DB cluster. When set to `limitless`, the cluster operates as an Aurora Limitless Database. When set to `standard` (the default), the cluster uses normal DB instance creation. Valid values: `limitless`, `standard`.
  /// [copyTagsToSnapshot] Copy all Cluster `tags` to snapshots. Default is `false`.
  /// [databaseInsightsMode] Mode of Database Insights to enable for the DB cluster. Valid values: `standard`, `advanced`.
  ...
  /// [dbClusterInstanceClass] Compute and memory capacity of each DB instance in the Multi-AZ DB cluster, for example `db.m6g.xlarge`. Not all DB instance classes are available in all AWS Regions, or for all database engines. For the full list of DB instance classes and availability for your engine, see [DB instance class](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.DBInstanceClass.html) in the Amazon RDS User Guide.
  /// [dbClusterParameterGroupName] Cluster parameter group to associate with the cluster.
  ...
  /// [dbSubnetGroupName] DB subnet group to associate with this DB cluster. **NOTE:** This must match the `dbSubnetGroupName` specified on every `aws.rds.ClusterInstance` in the cluster.
  /// [dbSystemId] For use with RDS Custom.
  /// [deleteAutomatedBackups] Whether to remove automated backups immediately after the DB cluster is deleted. Default is `true`.
  ...
  /// [domain] ID of the Directory Service Active Directory domain to create the cluster in.
  /// [domainIamRoleName] Name of the IAM role to be used when making API calls to the Directory Service.
  ...
  /// [engineLifecycleSupport] Life cycle type for this DB instance. This setting is valid for cluster types Aurora DB clusters and Multi-AZ DB clusters. Valid values are `open-source-rds-extended-support`, `open-source-rds-extended-support-disabled`. Default value is `open-source-rds-extended-support`. [Using Amazon RDS Extended Support]: https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/extended-support.html
  ...
  /// [iamDatabaseAuthenticationEnabled] Whether mappings of AWS Identity and Access Management (IAM) accounts to database accounts is enabled. Please see [AWS Documentation](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/UsingWithRDS.IAMDBAuth.html) for availability and limitations.
  ...
  /// [performanceInsightsKmsKeyId] KMS Key ID to encrypt Performance Insights data. If not specified, the default RDS KMS key will be used (`aws/rds`).
  ...
  /// [skipFinalSnapshot] Whether a final DB snapshot is created before the DB cluster is deleted. If true is specified, no DB snapshot is created. If false is specified, a DB snapshot is created before the DB cluster is deleted, using the value from `finalSnapshotIdentifier`. Default is `false`.
  /// [snapshotIdentifier] Whether to create this cluster from a snapshot. You can use either the name or ARN when specifying a DB cluster snapshot, or the ARN when specifying a DB snapshot. Conflicts with `globalClusterIdentifier`. Clusters cannot be restored from snapshot **and** joined to an existing global cluster in a single operation. See the [AWS documentation](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/aurora-global-database-getting-started.html#aurora-global-database.use-snapshot) or the Global Cluster Restored From Snapshot example for instructions on building a global cluster starting with a snapshot.
  /// [sourceRegion] Source region for an encrypted replica DB cluster.
  /// [storageEncrypted] Whether the DB cluster is encrypted. The default is `false` for `provisioned` `engineMode` and `true` for `serverless` `engineMode`. When restoring an unencrypted `snapshotIdentifier`, the `kmsKeyId` argument must be provided to encrypt the restored cluster. The provider will only perform drift detection if a configuration value is provided.
  ...
  factory ClusterArgs.fromMap(Map<String, dynamic> map) {
    return ClusterArgs(
      ...
      backtrackWindow: (() { final guardedValue = map['backtrackWindow']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      ...
      engine: pulumi.Input.fromValue(map['engine']),
      ...
      engineMode: (() { final guardedValue = map['engineMode']; if (guardedValue == null) return null; return pulumi.Input.fromValue(guardedValue); })(),
