...
class V2modelsIntentClosingSetting {
  ...
  /// [closingResponse] Configuration block for response that Amazon Lex sends to the user when the intent is complete. See `closingResponse`.
  /// [conditional] Configuration block for list of conditional branches associated with the intent's closing response. These branches are executed when the `nextStep` attribute is set to `EvalutateConditional`. See `conditional`.
  /// [nextStep] Next step that the bot executes after playing the intent's closing response. See `nextStep`.
