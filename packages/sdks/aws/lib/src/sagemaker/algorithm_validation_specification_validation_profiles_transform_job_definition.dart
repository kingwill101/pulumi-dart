...
class AlgorithmValidationSpecificationValidationProfilesTransformJobDefinition {
  ...
  factory AlgorithmValidationSpecificationValidationProfilesTransformJobDefinition.fromMap(Map<String, dynamic> map) {
    return AlgorithmValidationSpecificationValidationProfilesTransformJobDefinition(
      ...
      maxConcurrentTransforms: (() { final guardedValue = map['maxConcurrentTransforms']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
      maxPayloadInMb: (() { final guardedValue = map['maxPayloadInMb']; if (guardedValue == null) return null; return pulumi.Input.fromValue(((value) { final number = value as num; final integer = number.toInt(); if (number != integer) { throw FormatException('Expected an integer, got $number.'); } return integer; })(guardedValue)); })(),
