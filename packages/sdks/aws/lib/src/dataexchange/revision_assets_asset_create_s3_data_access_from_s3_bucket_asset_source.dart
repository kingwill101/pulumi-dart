// ignore_for_file: unused_element, unnecessary_cast

import 'package:pulumi/pulumi.dart' as pulumi;
import 'revision_assets_asset_create_s3_data_access_from_s3_bucket_asset_source_kms_keys_to_grant.dart';

class RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSource {
  /// The name of the S3 bucket.
  final pulumi.Input<String> bucket;
  /// List of key prefixes in the S3 bucket.
  final pulumi.Input<List<String>?>? keyPrefixes;
  /// List of object keys in the S3 bucket.
  final pulumi.Input<List<String>?>? keys;
  final pulumi.Input<List<RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSourceKmsKeysToGrant>?>? kmsKeysToGrants;

  /// Creates a new [RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSource].
  /// [bucket] The name of the S3 bucket.
  /// [keyPrefixes] List of key prefixes in the S3 bucket.
  /// [keys] List of object keys in the S3 bucket.
  /// [kmsKeysToGrants] Optional.
  const RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSource({
    required this.bucket,
    this.keyPrefixes,
    this.keys,
    this.kmsKeysToGrants,
  });

  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      'bucket': bucket,
      'keyPrefixes': ?keyPrefixes,
      'keys': ?keys,
      'kmsKeysToGrants': ?pulumi.Input.mapOptionalInputValue<List<RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSourceKmsKeysToGrant>, List<Map<String, dynamic>>>(kmsKeysToGrants, (value) => pulumi.Input.encodeList<RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSourceKmsKeysToGrant, Map<String, dynamic>>(value, (value) => value.toMap())),
    };
  }

  factory RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSource.fromMap(Map<String, dynamic> map) {
    return RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSource(
      bucket: pulumi.Input.fromValue(map['bucket'] as String),
      keyPrefixes: (() { final guardedValue = map['keyPrefixes']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as List).cast<String>()); })(),
      keys: (() { final guardedValue = map['keys']; if (guardedValue == null) return null; return pulumi.Input.fromValue((guardedValue as List).cast<String>()); })(),
      kmsKeysToGrants: (() { final guardedValue = map['kmsKeysToGrants']; if (guardedValue == null) return null; return pulumi.Input.fromValue(pulumi.Input.decodeList<RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSourceKmsKeysToGrant>(guardedValue, (value) => RevisionAssetsAssetCreateS3DataAccessFromS3BucketAssetSourceKmsKeysToGrant.fromMap((value as Map).cast<String, dynamic>()))); })(),
    );
  }
}
