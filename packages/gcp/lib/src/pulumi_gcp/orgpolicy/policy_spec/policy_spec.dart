...
class PolicySpec {
  ...
  /// Determines the inheritance behavior for this `Policy`. If <span pulumi-lang-nodejs="`inheritFromParent`" pulumi-lang-dotnet="`InheritFromParent`" pulumi-lang-go="`inheritFromParent`" pulumi-lang-python="`inherit_from_parent`" pulumi-lang-yaml="`inheritFromParent`" pulumi-lang-java="`inheritFromParent`">`inherit_from_parent`</span> is true, PolicyRules set higher up in the hierarchy (up to the closest root) are inherited and present in the effective policy. If it is false, then no rules are inherited, and this Policy becomes the new root for evaluation. This field can be set only for Policies which configure list constraints.
  ...
  /// In Policies for boolean constraints, the following requirements apply: - There must be one and only one PolicyRule where condition is unset. - BooleanPolicyRules with conditions must set <span pulumi-lang-nodejs="`enforced`" pulumi-lang-dotnet="`Enforced`" pulumi-lang-go="`enforced`" pulumi-lang-python="`enforced`" pulumi-lang-yaml="`enforced`" pulumi-lang-java="`enforced`">`enforced`</span> to the opposite of the PolicyRule without a condition. - During policy evaluation, PolicyRules with conditions that are true for a target resource take precedence.
