...
class BucketLoggingArgs {
  ...
  final Input<String> targetBucket;

  /// Set of configuration blocks with information for granting permissions. See below.
  final Input<List<BucketLoggingTargetGrant>>? targetGrants;

  /// Amazon S3 key format for log objects. See below.
  final Input<BucketLoggingTargetObjectKeyFormat>? targetObjectKeyFormat;

  /// Prefix for all log object keys.
  final Input<String> targetPrefix;
  ...
  Map<String, dynamic> toMap() {
    ...
    if (targetGrantsValue != null) {
      map['targetGrants'] = Input.mapOptionalInputValue<
              List<BucketLoggingTargetGrant>, List<Map<String, dynamic>>>(
          targetGrantsValue,
          (value) =>
              Input.encodeList<BucketLoggingTargetGrant, Map<String, dynamic>>(
                  value, (value) => value.toMap()));
    }
    final targetObjectKeyFormatValue = targetObjectKeyFormat;
    if (targetObjectKeyFormatValue != null) {
      map['targetObjectKeyFormat'] = Input.mapOptionalInputValue<
  ...
  factory BucketLoggingArgs.fromMap(Map<String, dynamic> map) {
    return BucketLoggingArgs(
      ...
      targetBucket: Input.asInput<String>(map['targetBucket']),
      targetGrants: Input.asOptionalInput<List<BucketLoggingTargetGrant>>(
          map['targetGrants']),
      targetObjectKeyFormat:
          Input.asOptionalInput<BucketLoggingTargetObjectKeyFormat>(
              map['targetObjectKeyFormat']),
      targetPrefix: Input.asInput<String>(map['targetPrefix']),
    );
  }
}
