// ignore_for_file: unused_element, unnecessary_cast

/// Result data returned by getPolicy.
class GetPolicyOrganizationsResult {
  /// The Amazon Resource Name of the policy.
  final String arn;

  /// Indicates if a policy is an AWS managed policy.
  final bool awsManaged;

  /// The text content of the policy.
  final String content;

  /// The description of the policy.
  final String description;

  /// The provider-assigned unique ID for this managed resource.
  final String id;

  /// The friendly name of the policy.
  final String name;
  final String policyId;

  /// The type of policy values can be `AISERVICES_OPT_OUT_POLICY | BACKUP_POLICY | CHATBOT_POLICY | DECLARATIVE_POLICY_EC2 | RESOURCE_CONTROL_POLICY | SERVICE_CONTROL_POLICY | TAG_POLICY`
  final String type;

  GetPolicyOrganizationsResult({
    required this.arn,
    required this.awsManaged,
    required this.content,
    required this.description,
    required this.id,
    required this.name,
    required this.policyId,
    required this.type,
  });

  Map<String, dynamic> toMap() {
    final map = <String, dynamic>{};
    map['arn'] = arn;
    map['awsManaged'] = awsManaged;
    map['content'] = content;
    map['description'] = description;
    map['id'] = id;
    map['name'] = name;
    map['policyId'] = policyId;
    map['type'] = type;
    return map;
  }

  factory GetPolicyOrganizationsResult.fromMap(Map<String, dynamic> map) {
    return GetPolicyOrganizationsResult(
      arn: map['arn'] as String,
      awsManaged: map['awsManaged'] as bool,
      content: map['content'] as String,
      description: map['description'] as String,
      id: map['id'] as String,
      name: map['name'] as String,
      policyId: map['policyId'] as String,
      type: map['type'] as String,
    );
  }
}
