...
class ClusterMasterInstanceFleetLaunchSpecificationsSpotSpecification {
  ...
  /// Creates a new [ClusterMasterInstanceFleetLaunchSpecificationsSpotSpecification].
  /// [allocationStrategy] Specifies the strategy to use in launching Spot instance fleets. Valid values include `capacity-optimized`, `diversified`, `lowest-price`, `price-capacity-optimized`. See the [AWS documentation](https://docs.aws.amazon.com/emr/latest/ManagementGuide/emr-instance-fleet.html#emr-instance-fleet-allocation-strategy) for details on each strategy type.
  /// [blockDurationMinutes] Defined duration for Spot instances (also known as Spot blocks) in minutes. When specified, the Spot instance does not terminate before the defined duration expires, and defined duration pricing for Spot instances applies. Valid values are 60, 120, 180, 240, 300, or 360. The duration period starts as soon as a Spot instance receives its instance ID. At the end of the duration, Amazon EC2 marks the Spot instance for termination and provides a Spot instance termination notice, which gives the instance a two-minute warning before it terminates.
  ...
  /// [timeoutDurationMinutes] Spot provisioning timeout period in minutes. If Spot instances are not provisioned within this time period, the TimeOutAction is taken. Minimum value is 5 and maximum value is 1440. The timeout applies only during initial provisioning, when the cluster is first created.
  ...
  Map<String, dynamic> toMap() {
    ...
    map['timeoutAction'] = timeoutAction;
    map['timeoutDurationMinutes'] = timeoutDurationMinutes;
    return map;
  }

  factory ClusterMasterInstanceFleetLaunchSpecificationsSpotSpecification.fromMap(
