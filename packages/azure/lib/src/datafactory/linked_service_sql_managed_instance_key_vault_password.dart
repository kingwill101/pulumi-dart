// ignore_for_file: unused_element, unnecessary_cast


class LinkedServiceSqlManagedInstanceKeyVaultPassword {
  /// Specifies the name of an existing Key Vault Data Factory Linked Service.
  final String linkedServiceName;
  /// Specifies the secret name in Azure Key Vault that stores SQL Managed Instance password.
  final String secretName;

  /// Creates a new [LinkedServiceSqlManagedInstanceKeyVaultPassword].
  /// [linkedServiceName] Specifies the name of an existing Key Vault Data Factory Linked Service.
  /// [secretName] Specifies the secret name in Azure Key Vault that stores SQL Managed Instance password.
  LinkedServiceSqlManagedInstanceKeyVaultPassword({
    required this.linkedServiceName,
    required this.secretName,
  });

  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      'linkedServiceName': linkedServiceName,
      'secretName': secretName,
    };
  }

  factory LinkedServiceSqlManagedInstanceKeyVaultPassword.fromMap(Map<String, dynamic> map) {
    return LinkedServiceSqlManagedInstanceKeyVaultPassword(
      linkedServiceName: map['linkedServiceName'] as String,
      secretName: map['secretName'] as String,
    );
  }
}

