...
import 'app_engine_http_request_http_method.dart';
...
class AppEngineHttpRequest {
  ...
  final AppEngineHttpRequestHttpMethod? httpMethod;

  /// The relative URI. The relative URI must begin with "/" and must be a valid HTTP relative URI. It can contain a path and query string arguments. If the relative URI is empty, then the root path "/" will be used. No spaces are allowed, and the maximum length allowed is 2083 characters.
  final String? relativeUri;

  /// Creates a new [AppEngineHttpRequest].
  /// [appEngineRouting] Task-level setting for App Engine routing. * If app_engine_routing_override is set on the queue, this value is used for all tasks in the queue, no matter what the setting is for the task-level app_engine_routing.
  ...
  factory AppEngineHttpRequest.fromMap(Map<String, dynamic> map) {
    return AppEngineHttpRequest(
      ...
      httpMethod: map['httpMethod'] == null
          ? null
          : AppEngineHttpRequestHttpMethod.fromValue(
              map['httpMethod'] as String,
            ),
      relativeUri: map['relativeUri'] == null
          ? null
          : map['relativeUri'] as String,
    );
  }
}
