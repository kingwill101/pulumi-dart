...
class GoogleCloudContentwarehouseV1Action {
  ...
  Map<String, dynamic> toMap() {
    ...
    final accessControlValue = accessControl;
    if (accessControlValue != null) {
      map['accessControl'] = accessControlValue.toMap();
    }
    final actionIdValue = actionId;
    if (actionIdValue != null) {
      map['actionId'] = actionIdValue;
    }
    final addToFolderValue = addToFolder;
    if (addToFolderValue != null) {
      map['addToFolder'] = addToFolderValue.toMap();
    }
    final dataUpdateValue = dataUpdate;
    if (dataUpdateValue != null) {
      map['dataUpdate'] = dataUpdateValue.toMap();
    }
    final dataValidationValue = dataValidation;
    if (dataValidationValue != null) {
      map['dataValidation'] = dataValidationValue.toMap();
    }
    final deleteDocumentActionValue = deleteDocumentAction;
    if (deleteDocumentActionValue != null) {
      map['deleteDocumentAction'] = deleteDocumentActionValue.toMap();
    }
    final publishToPubSubValue = publishToPubSub;
    if (publishToPubSubValue != null) {
      map['publishToPubSub'] = publishToPubSubValue.toMap();
    }
    final removeFromFolderActionValue = removeFromFolderAction;
    if (removeFromFolderActionValue != null) {
      map['removeFromFolderAction'] = removeFromFolderActionValue.toMap();
    }
    return map;
  }
