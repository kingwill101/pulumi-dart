...
import 'http_header_match_response3.dart';
import 'http_query_parameter_match_response3.dart';
import 'metadata_filter_response3.dart';

/// HttpRouteRuleMatch specifies a set of criteria for matching requests to an HttpRouteRule. All specified criteria must be satisfied for a match to occur.
class HttpRouteRuleMatchResponse3 {
  /// For satisfying the matchRule condition, the path of the request must exactly match the value specified in fullPathMatch after removing any query parameters and anchor that may be part of the original URL. fullPathMatch must be from 1 to 1024 characters. Only one of prefixMatch, fullPathMatch or regexMatch must be specified.
  final String fullPathMatch;

  /// Specifies a list of header match criteria, all of which must match corresponding headers in the request.
  final List<HttpHeaderMatchResponse3> headerMatches;
  ...
  final List<MetadataFilterResponse3> metadataFilters;
  ...
  final List<HttpQueryParameterMatchResponse3> queryParameterMatches;

  /// For satisfying the matchRule condition, the path of the request must satisfy the regular expression specified in regexMatch after removing any query parameters and anchor supplied with the original URL. For more information about regular expression syntax, see Syntax. Only one of prefixMatch, fullPathMatch or regexMatch must be specified. Regular expressions can only be used when the loadBalancingScheme is set to INTERNAL_SELF_MANAGED.
  final String regexMatch;

  HttpRouteRuleMatchResponse3({
  ...
  Map<String, dynamic> toMap() {
    ...
    map['headerMatches'] =
        Input.encodeList<HttpHeaderMatchResponse3, Map<String, dynamic>>(
            headerMatches, (value) => value.toMap());
    map['ignoreCase'] = ignoreCase;
    map['metadataFilters'] =
        Input.encodeList<MetadataFilterResponse3, Map<String, dynamic>>(
    ...
    map['queryParameterMatches'] = Input.encodeList<
        HttpQueryParameterMatchResponse3,
        Map<String, dynamic>>(queryParameterMatches, (value) => value.toMap());
    map['regexMatch'] = regexMatch;
    return map;
  }

  factory HttpRouteRuleMatchResponse3.fromMap(Map<String, dynamic> map) {
    return HttpRouteRuleMatchResponse3(
      fullPathMatch: map['fullPathMatch'] as String,
      headerMatches: Input.decodeList<HttpHeaderMatchResponse3>(
          map['headerMatches'],
          (value) => HttpHeaderMatchResponse3.fromMap(
              (value as Map).cast<String, dynamic>())),
      ignoreCase: map['ignoreCase'] as bool,
      metadataFilters: Input.decodeList<MetadataFilterResponse3>(
          map['metadataFilters'],
          (value) => MetadataFilterResponse3.fromMap(
              (value as Map).cast<String, dynamic>())),
      pathTemplateMatch: map['pathTemplateMatch'] as String,
      prefixMatch: map['prefixMatch'] as String,
      queryParameterMatches: Input.decodeList<HttpQueryParameterMatchResponse3>(
          map['queryParameterMatches'],
          (value) => HttpQueryParameterMatchResponse3.fromMap(
              (value as Map).cast<String, dynamic>())),
