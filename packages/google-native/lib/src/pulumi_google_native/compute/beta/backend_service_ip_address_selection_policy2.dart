...
enum BackendServiceIpAddressSelectionPolicy2 {
  ipv4Only("IPV4_ONLY"),
  ipv6Only("IPV6_ONLY"),
  ...
  const BackendServiceIpAddressSelectionPolicy2(this.value);
  final String value;

  static BackendServiceIpAddressSelectionPolicy2 fromValue(String value) {
    for (final item in BackendServiceIpAddressSelectionPolicy2.values) {
      if (item.value == value) {
        return item;
      }
    }
    throw ArgumentError(
        'Unknown BackendServiceIpAddressSelectionPolicy2 value: $value');
  }
}
