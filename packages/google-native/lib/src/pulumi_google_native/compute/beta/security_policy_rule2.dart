...
import 'security_policy_rule_direction2.dart';
import 'security_policy_rule_http_header_action2.dart';
import 'security_policy_rule_matcher2.dart';
import 'security_policy_rule_network_matcher2.dart';
import 'security_policy_rule_preconfigured_waf_config2.dart';
import 'security_policy_rule_rate_limit_options2.dart';
import 'security_policy_rule_redirect_options2.dart';

/// Represents a rule that describes one or more match conditions along with the action to be taken when traffic matches this condition (allow or deny).
class SecurityPolicyRule2 {
  ...
  final SecurityPolicyRuleDirection2? direction;

  /// Denotes whether to enable logging for a particular rule. If logging is enabled, logs will be exported to the configured export destination in Stackdriver. Logs may be exported to BigQuery or Pub/Sub. Note: you cannot enable logging on "goto_next" rules. This field may only be specified when the versioned_expr is set to FIREWALL.
  final bool? enableLogging;

  /// Optional, additional actions that are performed on headers. This field is only supported in Global Security Policies of type CLOUD_ARMOR.
  final SecurityPolicyRuleHttpHeaderAction2? headerAction;

  /// A match condition that incoming traffic is evaluated against. If it evaluates to true, the corresponding 'action' is enforced.
  final SecurityPolicyRuleMatcher2? match;
  ...
  final SecurityPolicyRuleNetworkMatcher2? networkMatch;

  /// Preconfigured WAF configuration to be applied for the rule. If the rule does not evaluate preconfigured WAF rules, i.e., if evaluatePreconfiguredWaf() is not used, this field will have no effect.
  final SecurityPolicyRulePreconfiguredWafConfig2? preconfiguredWafConfig;
  ...
  final SecurityPolicyRuleRateLimitOptions2? rateLimitOptions;

  /// Parameters defining the redirect action. Cannot be specified for any other actions. This field is only supported in Global Security Policies of type CLOUD_ARMOR.
  final SecurityPolicyRuleRedirectOptions2? redirectOptions;
  ...
  factory SecurityPolicyRule2.fromMap(Map<String, dynamic> map) {
    return SecurityPolicyRule2(
      ...
      rateLimitOptions: map['rateLimitOptions'] == null
          ? null
          : SecurityPolicyRuleRateLimitOptions2.fromMap(
              (map['rateLimitOptions'] as Map).cast<String, dynamic>()),
      redirectOptions: map['redirectOptions'] == null
          ? null
          : SecurityPolicyRuleRedirectOptions2.fromMap(
              (map['redirectOptions'] as Map).cast<String, dynamic>()),
      ruleNumber:
          map['ruleNumber'] == null ? null : map['ruleNumber'] as String,
