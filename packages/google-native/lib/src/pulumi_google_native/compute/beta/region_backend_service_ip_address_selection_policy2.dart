...
enum RegionBackendServiceIpAddressSelectionPolicy2 {
  ipv4Only("IPV4_ONLY"),
  ipv6Only("IPV6_ONLY"),
  ...
  const RegionBackendServiceIpAddressSelectionPolicy2(this.value);
  final String value;

  static RegionBackendServiceIpAddressSelectionPolicy2 fromValue(String value) {
    for (final item in RegionBackendServiceIpAddressSelectionPolicy2.values) {
      if (item.value == value) {
        return item;
      }
    }
    throw ArgumentError(
        'Unknown RegionBackendServiceIpAddressSelectionPolicy2 value: $value');
  }
}
