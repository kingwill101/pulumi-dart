...
import 'app_engine_http_target_http_method2.dart';
import 'app_engine_routing2.dart';

/// App Engine target. The job will be pushed to a job handler by means of an HTTP request via an http_method such as HTTP POST, HTTP GET, etc. The job is acknowledged by means of an HTTP response code in the range [200 - 299]. Error 503 is considered an App Engine system error instead of an application error. Requests returning error 503 will be retried regardless of retry configuration and not counted against retry counts. Any other response code, or a failure to receive a response before the deadline, constitutes a failed attempt.
class AppEngineHttpTarget2 {
  /// App Engine Routing setting for the job.
  final AppEngineRouting2? appEngineRouting;
  ...
  final AppEngineHttpTargetHttpMethod2? httpMethod;

  /// The relative URI. The relative URL must begin with "/" and must be a valid HTTP relative URL. It can contain a path, query string arguments, and `#` fragments. If the relative URL is empty, then the root path "/" will be used. No spaces are allowed, and the maximum length allowed is 2083 characters.
  final String? relativeUri;

  AppEngineHttpTarget2({
  ...
  factory AppEngineHttpTarget2.fromMap(Map<String, dynamic> map) {
    return AppEngineHttpTarget2(
      appEngineRouting: map['appEngineRouting'] == null
          ? null
          : AppEngineRouting2.fromMap(
      ...
      httpMethod: map['httpMethod'] == null
          ? null
          : AppEngineHttpTargetHttpMethod2.fromValue(
