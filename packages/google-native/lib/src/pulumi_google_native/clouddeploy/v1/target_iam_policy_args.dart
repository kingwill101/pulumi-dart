...
import 'audit_config_clouddeploy_v1.dart';
import 'binding_clouddeploy_v1.dart';

/// The set of arguments for TargetIamPolicy.
class TargetIamPolicyArgs {
  /// Specifies cloud audit logging configuration for this policy.
  final pulumi.Input<List<AuditConfigClouddeployV1>>? auditConfigs;

  /// Associates a list of `members`, or principals, with a `role`. Optionally, may specify a `condition` that determines how and when the `bindings` are applied. Each of the `bindings` must contain at least one principal. The `bindings` in a `Policy` can refer to up to 1,500 principals; up to 250 of these principals can be Google groups. Each occurrence of a principal counts towards these limits. For example, if the `bindings` grant 50 different roles to `user:alice@example.com`, and not to any other principal, then you can add another 1,450 principals to the `bindings` in the `Policy`.
  final pulumi.Input<List<BindingClouddeployV1>>? bindings;
  ...
  Map<String, dynamic> toMap() {
    ...
    if (auditConfigsValue != null) {
      map['auditConfigs'] = pulumi.Input.mapOptionalInputValue<
              List<AuditConfigClouddeployV1>, List<Map<String, dynamic>>>(
          auditConfigsValue,
          (value) => pulumi.Input.encodeList<AuditConfigClouddeployV1,
              Map<String, dynamic>>(value, (value) => value.toMap()));
    }
    final bindingsValue = bindings;
    if (bindingsValue != null) {
      map['bindings'] = pulumi.Input.mapOptionalInputValue<
              List<BindingClouddeployV1>, List<Map<String, dynamic>>>(
          bindingsValue,
          (value) => pulumi.Input.encodeList<BindingClouddeployV1,
              Map<String, dynamic>>(value, (value) => value.toMap()));
    }
  ...
  factory TargetIamPolicyArgs.fromMap(Map<String, dynamic> map) {
    return TargetIamPolicyArgs(
      auditConfigs:
          pulumi.Input.asOptionalInput<List<AuditConfigClouddeployV1>>(
              map['auditConfigs']),
      bindings: pulumi.Input.asOptionalInput<List<BindingClouddeployV1>>(
          map['bindings']),
