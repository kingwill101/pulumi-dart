// ignore_for_file: unused_element, unnecessary_cast

import 'package:pulumi/pulumi.dart' hide Config;
import 'apigateway_api_config_file2.dart';

/// A gRPC service definition.
class ApigatewayApiConfigGrpcServiceDefinition2 {
  /// Input only. File descriptor set, generated by protoc. To generate, use protoc with imports and source info included. For an example test.proto file, the following command would put the value in a new file named out.pb. $ protoc --include_imports --include_source_info test.proto -o out.pb
  final ApigatewayApiConfigFile2? fileDescriptorSet;

  /// Optional. Uncompiled proto files associated with the descriptor set, used for display purposes (server-side compilation is not supported). These should match the inputs to 'protoc' command used to generate file_descriptor_set.
  final List<ApigatewayApiConfigFile2>? source;

  ApigatewayApiConfigGrpcServiceDefinition2({
    this.fileDescriptorSet,
    this.source,
  });

  Map<String, dynamic> toMap() {
    final map = <String, dynamic>{};
    final fileDescriptorSetValue = fileDescriptorSet;
    if (fileDescriptorSetValue != null) {
      map['fileDescriptorSet'] = fileDescriptorSetValue.toMap();
    }
    final sourceValue = source;
    if (sourceValue != null) {
      map['source'] =
          Input.encodeList<ApigatewayApiConfigFile2, Map<String, dynamic>>(
              sourceValue, (value) => value.toMap());
    }
    return map;
  }

  factory ApigatewayApiConfigGrpcServiceDefinition2.fromMap(
      Map<String, dynamic> map) {
    return ApigatewayApiConfigGrpcServiceDefinition2(
      fileDescriptorSet: map['fileDescriptorSet'] == null
          ? null
          : ApigatewayApiConfigFile2.fromMap(
              (map['fileDescriptorSet'] as Map).cast<String, dynamic>()),
      source: map['source'] == null
          ? null
          : Input.decodeList<ApigatewayApiConfigFile2>(
              map['source'],
              (value) => ApigatewayApiConfigFile2.fromMap(
                  (value as Map).cast<String, dynamic>())),
    );
  }
}
