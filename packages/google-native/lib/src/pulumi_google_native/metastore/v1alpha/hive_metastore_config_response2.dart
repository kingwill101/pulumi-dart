// ignore_for_file: unused_element, unnecessary_cast

import 'kerberos_config_response4.dart';

/// Specifies configuration information specific to running Hive metastore software as the metastore service.
class HiveMetastoreConfigResponse2 {
  /// A mapping of Hive metastore version to the auxiliary version configuration. When specified, a secondary Hive metastore service is created along with the primary service. All auxiliary versions must be less than the service's primary version. The key is the auxiliary service name and it must match the regular expression a-z?. This means that the first character must be a lowercase letter, and all the following characters must be hyphens, lowercase letters, or digits, except the last character, which cannot be a hyphen.
  final Map<String, String> auxiliaryVersions;

  /// A mapping of Hive metastore configuration key-value pairs to apply to the Hive metastore (configured in hive-site.xml). The mappings override system defaults (some keys cannot be overridden). These overrides are also applied to auxiliary versions and can be further customized in the auxiliary version's AuxiliaryVersionConfig.
  final Map<String, String> configOverrides;

  /// The protocol to use for the metastore service endpoint. If unspecified, defaults to THRIFT.
  final String endpointProtocol;

  /// Information used to configure the Hive metastore service as a service principal in a Kerberos realm. To disable Kerberos, use the UpdateService method and specify this field's path (hive_metastore_config.kerberos_config) in the request's update_mask while omitting this field from the request's service.
  final KerberosConfigResponse4 kerberosConfig;

  /// Immutable. The Hive metastore schema version.
  final String version;

  HiveMetastoreConfigResponse2({
    required this.auxiliaryVersions,
    required this.configOverrides,
    required this.endpointProtocol,
    required this.kerberosConfig,
    required this.version,
  });

  Map<String, dynamic> toMap() {
    final map = <String, dynamic>{};
    map['auxiliaryVersions'] = auxiliaryVersions;
    map['configOverrides'] = configOverrides;
    map['endpointProtocol'] = endpointProtocol;
    map['kerberosConfig'] = kerberosConfig.toMap();
    map['version'] = version;
    return map;
  }

  factory HiveMetastoreConfigResponse2.fromMap(Map<String, dynamic> map) {
    return HiveMetastoreConfigResponse2(
      auxiliaryVersions:
          (map['auxiliaryVersions'] as Map).cast<String, String>(),
      configOverrides: (map['configOverrides'] as Map).cast<String, String>(),
      endpointProtocol: map['endpointProtocol'] as String,
      kerberosConfig: KerberosConfigResponse4.fromMap(
          (map['kerberosConfig'] as Map).cast<String, dynamic>()),
      version: map['version'] as String,
    );
  }
}
