...
import 'http_header_match_networksecurity_v1.dart';

/// Specification of traffic destination attributes.
class DestinationNetworksecurityV1 {
  /// List of host names to match. Matched against the ":authority" header in http requests. At least one host should match. Each host can be an exact match, or a prefix match (example "mydomain.*") or a suffix match (example "*.myorg.com") or a presence (any) match "*".
  final List<String> hosts;

  /// Optional. Match against key:value pair in http header. Provides a flexible match based on HTTP headers, for potentially advanced use cases. At least one header should match. Avoid using header matches to make authorization decisions unless there is a strong guarantee that requests arrive through a trusted client or proxy.
  final HttpHeaderMatchNetworksecurityV1? httpHeaderMatch;
  ...
  factory DestinationNetworksecurityV1.fromMap(Map<String, dynamic> map) {
    return DestinationNetworksecurityV1(
      hosts: (map['hosts'] as List).cast<String>(),
      httpHeaderMatch: map['httpHeaderMatch'] == null
          ? null
          : HttpHeaderMatchNetworksecurityV1.fromMap(
