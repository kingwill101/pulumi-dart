...
import 'autoscaling_config2.dart';
import 'encryption_config9.dart';
import 'endpoint_config2.dart';
import 'gce_cluster_config2.dart';
import 'gke_cluster_config2.dart';
import 'instance_group_config2.dart';
import 'lifecycle_config2.dart';
import 'metastore_config2.dart';
import 'node_initialization_action2.dart';
import 'security_config2.dart';
import 'software_config4.dart';

/// The cluster config.
class ClusterConfig3 {
  /// Optional. Autoscaling config for the policy associated with the cluster. Cluster does not autoscale if this field is unset.
  final AutoscalingConfig2? autoscalingConfig;
  ...
  final EncryptionConfig9? encryptionConfig;

  /// Optional. Port/endpoint configuration for this cluster
  final EndpointConfig2? endpointConfig;

  /// Optional. The shared Compute Engine config settings for all instances in a cluster.
  final GceClusterConfig2? gceClusterConfig;
  ...
  final GkeClusterConfig2? gkeClusterConfig;

  /// Optional. Commands to execute on each node after config is completed. By default, executables are run on master and all worker nodes. You can test a node's role metadata to run an executable on a master or worker node, as shown below using curl (you can also use wget): ROLE=$(curl -H Metadata-Flavor:Google http://metadata/computeMetadata/v1beta2/instance/attributes/dataproc-role) if [[ "${ROLE}" == 'Master' ]]; then ... master specific actions ... else ... worker specific actions ... fi
  final List<NodeInitializationAction2>? initializationActions;

  /// Optional. The config setting for auto delete cluster schedule.
  final LifecycleConfig2? lifecycleConfig;

  /// Optional. The Compute Engine config settings for the master instance in a cluster.
  final InstanceGroupConfig2? masterConfig;

  /// Optional. Metastore configuration.
  final MetastoreConfig2? metastoreConfig;

  /// Optional. The Compute Engine config settings for additional worker instances in a cluster.
  final InstanceGroupConfig2? secondaryWorkerConfig;

  /// Optional. Security related configuration.
  final SecurityConfig2? securityConfig;

  /// Optional. The config settings for software inside the cluster.
  final SoftwareConfig4? softwareConfig;
  ...
  final InstanceGroupConfig2? workerConfig;
  ...
  Map<String, dynamic> toMap() {
    ...
    if (initializationActionsValue != null) {
      map['initializationActions'] =
          Input.encodeList<NodeInitializationAction2, Map<String, dynamic>>(
  ...
  factory ClusterConfig3.fromMap(Map<String, dynamic> map) {
    return ClusterConfig3(
      autoscalingConfig: map['autoscalingConfig'] == null
          ? null
          : AutoscalingConfig2.fromMap(
