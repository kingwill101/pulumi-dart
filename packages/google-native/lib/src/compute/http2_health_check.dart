...
class HTTP2HealthCheck {
  ...
  final HTTP2HealthCheckPortSpecification? portSpecification;
  /// Specifies the type of proxy header to append before sending data to the backend, either NONE or PROXY_V1. The default is NONE.
  final HTTP2HealthCheckProxyHeader? proxyHeader;
  /// The request path of the HTTP/2 health check request. The default value is /.
  final String? requestPath;
  /// Creates a content-based HTTP/2 health check. In addition to the required HTTP 200 (OK) status code, you can configure the health check to pass only when the backend sends this specific ASCII response string within the first 1024 bytes of the HTTP response body. For details, see: https://cloud.google.com/load-balancing/docs/health-check-concepts#criteria-protocol-http
  final String? response;
  /// Weight report mode. used for weighted Load Balancing.
  final HTTP2HealthCheckWeightReportMode? weightReportMode;
  ...
  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      ...
      'response': ?response,
      'weightReportMode': ?weightReportMode == null ? null : weightReportMode!.value,
    };
  }
  ...
  factory HTTP2HealthCheck.fromMap(Map<String, dynamic> map) {
    return HTTP2HealthCheck(
      ...
      portSpecification: map['portSpecification'] == null ? null : HTTP2HealthCheckPortSpecification.fromValue(map['portSpecification'] as String),
      proxyHeader: map['proxyHeader'] == null ? null : HTTP2HealthCheckProxyHeader.fromValue(map['proxyHeader'] as String),
      requestPath: map['requestPath'] == null ? null : map['requestPath'] as String,
      response: map['response'] == null ? null : map['response'] as String,
      weightReportMode: map['weightReportMode'] == null ? null : HTTP2HealthCheckWeightReportMode.fromValue(map['weightReportMode'] as String),
    );
  }
}
