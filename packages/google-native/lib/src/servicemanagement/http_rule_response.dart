...
import 'custom_http_pattern_response.dart';
...
class HttpRuleResponse {
  ...
  /// Creates a new [HttpRuleResponse].
  /// [additionalBindings] Additional HTTP bindings for the selector. Nested bindings must not contain an `additional_bindings` field themselves (that is, the nesting may only be one level deep).
  /// [body] The name of the request field whose value is mapped to the HTTP request body, or `*` for mapping all request fields not captured by the path pattern to the HTTP body, or omitted for not having any HTTP request body. NOTE: the referred field must be present at the top-level of the request message type.
  /// [custom] The custom pattern is used for specifying an HTTP method that is not included in the `pattern` field, such as HEAD, or "*" to leave the HTTP method unspecified for this rule. The wild-card rule is useful for services that provide content to Web (HTML) clients.
  /// [delete] Maps to HTTP DELETE. Used for deleting a resource.
  /// [get] Maps to HTTP GET. Used for listing and getting information about resources.
  ...
  Map<String, dynamic> toMap() {
    ...
    map['additionalBindings'] =
        pulumi.Input.encodeList<HttpRuleResponse, Map<String, dynamic>>(
            additionalBindings, (value) => value.toMap());
    map['body'] = body;
