...
class CommandEc2CopyfileStack extends pulumi.Stack {
  CommandEc2CopyfileStack() : super() {
    ...
    final copy = command.remote.CopyFile(
      'copy',
      args: command.remote.CopyFileArgs(
        connection: connection.input(),
        localPath: 'src/file1'.input(),
        remotePath: destDir.input(),
      ),
      options: pulumi.CustomResourceOptions(dependsOn: [poll]),
    );

    final triggerToken = File('src/file1').readAsStringSync();
