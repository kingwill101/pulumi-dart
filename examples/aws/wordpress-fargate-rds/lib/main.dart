...
Future<void> run() async {
  ...
  final availabilityZones = await aws.index.getAvailabilityZones(
  ...
  final subnet0 = aws.ec2.Subnet(
    ...
    args: aws.ec2.SubnetArgs(
      ...
      availabilityZone: availabilityZones.names[0].input(),
  ...
  final subnet1 = aws.ec2.Subnet(
    ...
    args: aws.ec2.SubnetArgs(
      ...
      availabilityZone: availabilityZones.names[1].input(),
  ...
  final subnetIds = [subnet0.id, subnet1.id].input();
  ...
  final db = aws.rds.Instance(
    ...
    args: aws.rds.InstanceArgs(
      ...
      password: dbPassword,
      vpcSecurityGroupIds: [rdsSecurityGroup.id].input(),
  ...
  final alb = aws.lb.LoadBalancer(
    ...
    args: aws.lb.LoadBalancerArgs(
      ...
      securityGroups: [feSecurityGroup.id].input(),
      subnets: subnetIds,
    ),
  );
  ...
  pulumi.export('webServiceUrl', alb.dnsName.apply((dns) => 'http://${dns ?? ''}'));
  pulumi.export('ecsClusterName', cluster.name);
  pulumi.export('databaseEndpoint', db.address);
  pulumi.export('databaseUserName', db.username);
  pulumi.export('databasePassword', dbPassword);
}
