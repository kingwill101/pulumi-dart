...
class Converted extends pulumi.ComponentResource {
  ...
  Converted(String name, {required ConvertedArgs args, bool registerInputs = true, pulumi.ComponentResourceOptions? options})
      : super('components:index:Converted', name, registerInputs ? pulumi.Input.mapToInputs(args.toMap()) : const {}, options) {
...
class GeneratedStack extends pulumi.Stack {
  ...
  GeneratedStack() {
    ...
    final converted = Converted('converted', args: ConvertedArgs(boolean: pulumi.Input.asInput(pulumi.output('false').apply<bool>((value) => bool.parse(value))), float: pulumi.Input.asInput(pulumi.output('2.5').apply<double>((value) => double.parse(value))), integer: pulumi.Input.asInput(pulumi.output('7').apply<int>((value) => int.parse(value))), string_: pulumi.Input.asInput(pulumi.output(true).apply<String>((value) => (value).toString())), numberArray: pulumi.Input.asInput(pulumi.output(['10', '11']).apply<List<double>>((value) => (value as List).map<double>((item) => item is String ? double.parse(item) : (item as num).toDouble()).toList())), booleanMap: pulumi.Input.asInput(pulumi.output({'left': 'true', 'right': 'false'}).apply<Map<String, bool>>((value) => Map<String, bool>.fromEntries((value as Map).entries.map((entry) => MapEntry(entry.key.toString(), entry.value is String ? bool.parse(entry.value) : entry.value as bool))))), ));
