...
class GeneratedStack extends pulumi.Stack {
  ...
  Future<void> initialize() async {
    ...
    final routes = [for (final range in await pulumi.resolveRangeEntries(pulumi.output(values.apply((value) => value.results)).apply<int>((value) => (value as dynamic).length as int))) pulumi_nestedobject_index.Target('routes-' + range.key.toString(), args: pulumi_nestedobject_index.TargetArgs(name: pulumi.Input.asInput(pulumi.output(pulumi.output([values.apply((value) => value.results), range.value]).apply<dynamic>((values) => pulumi.indexValue(values[0], values[1]))).apply<String>((value) => (value).toString())), ))];
