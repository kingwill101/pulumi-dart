...
class First extends pulumi.ComponentResource {
  ...
  First(String name, {required FirstArgs args, bool registerInputs = true, pulumi.ComponentResourceOptions? options})
      : super('components:index:First', name, registerInputs ? pulumi.Input.mapToInputs(args.toMap()) : const {}, options) {
...
class Second extends pulumi.ComponentResource {
  ...
  Second(String name, {required SecondArgs args, bool registerInputs = true, pulumi.ComponentResourceOptions? options})
      : super('components:index:Second', name, registerInputs ? pulumi.Input.mapToInputs(args.toMap()) : const {}, options) {
...
class GeneratedStack extends pulumi.Stack {
  ...
  GeneratedStack() {
    ...
    final first = First('first', args: FirstArgs(input: pulumi.Input.asInput(pulumi.output(firstSecondUntainted).apply<bool>((value) => value as bool)), ), registerInputs: false);
    ...
    final another = First('another', args: FirstArgs(input: pulumi.Input.asInput(pulumi.output(pulumi.output([pulumi.output(['', anotherLoopingOverMany]).apply<String>((values) => (values[1] as Iterable).join(values[0].toString())), 'xyz']).apply<dynamic>((values) => (values[0] == values[1]))).apply<bool>((value) => value as bool)), ), registerInputs: false);
